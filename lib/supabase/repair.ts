...
export class SupabaseRepair {
    ...
    try {
      ...
      for (const user of authUsers.users) {
        ...
        if (profileError && profileError.code === 'PGRST116') {
          ...
          if (insertError) {
            console.warn(`Failed to create profile for ${user.email}:`, insertError);
          } else {
  ...
  async cleanOrphanedData() {
    ...
    try {
      ...
      const { error: capsuleCleanError } = await supabase.rpc('clean_orphaned_capsules');
      if (capsuleCleanError && !capsuleCleanError.message.includes('function')) {
        console.warn('Capsule cleanup warning:', capsuleCleanError);
      }
      
      // Clean transaction history without valid users  
      const { error: txCleanError } = await supabase.rpc('clean_orphaned_transactions');
