...
export const GUARDIANCHAIN_MODULES: ModuleInfo[] = [
  // Core Authentication System
  {
    ...
    description:
      "Production-ready debug authentication system with tier-based access control",
    category: "auth",
...
export const MODULE_STATS = {
  ...
  byCategory: GUARDIANCHAIN_MODULES.reduce(
    (acc, module) => {
  ...
  highPriority: GUARDIANCHAIN_MODULES.filter((m) => m.grantRelevance === "high")
    .length,
  coreModules: GUARDIANCHAIN_MODULES.filter(
    (m) => m.category === "core" || m.category === "auth",
  ).length,
};
...
export function getGrantRelevantModules(): ModuleInfo[] {
  return GUARDIANCHAIN_MODULES.filter(
    (module) => module.grantRelevance === "high",
  );
}
