...
function initializeWeb3() {
  try {
    ...
    if (!signer && ETH_PRIVATE_KEY.length === 66 && ETH_PRIVATE_KEY.startsWith('0x')) {
    ...
    console.warn('⚠️ Web3 initialization failed (development mode):', error instanceof Error ? error.message : 'Unknown error');
...
export async function triggerGTTYield(author: string, griefTier: number) {
  ...
  if (!web3.contract) {
    throw new Error('Web3 not properly initialized - missing contract or signer');
  }
...
export async function distributeReplayYield(
  ...
  try {
    ...
    const authorTx = await web3.contract.distributeYield(authorAddress, authorShare);
    await authorTx.wait();
    console.log('✅ Author yield distributed:', authorTx.hash);

    // Distribute to viewer (replay reward)
    const viewerTx = await web3.contract.distributeYield(viewerAddress, viewerShare);
    await viewerTx.wait();
    ...
    throw new Error(`GTT yield distribution failed: ${error instanceof Error ? error.message : 'Unknown error'}`);
  }
}
...
export async function calculateGriefYield(
  ...
  const ageBonus = Math.min(1.5, capsuleAge / (30 * 24 * 60 * 60 * 1000) * 0.1); // Age bonus (max 1.5x)
  
  const totalYield = baseYield * truthMultiplier * (1 + verificationBonus) * (1 + ageBonus);
...
export async function getContractInfo() {
  const web3 = initializeWeb3();
  if (!web3.contract) {
    console.warn('⚠️ Web3 contract not available, returning mock contract info');
    return {
