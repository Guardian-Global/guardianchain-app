...
const JWT_SECRET = process.env.JWT_SECRET || 'guardianchain-production-secret-change-in-prod';
...
// Update stake amount
router.patch('/update-stake', authenticateToken, (req: Request, res: Response) => {
  try {
    const { userId } = (req as any).user;
...
// Upgrade tier
router.post('/upgrade-tier', authenticateToken, (req: Request, res: Response) => {
  try {
    const { userId } = (req as any).user;
