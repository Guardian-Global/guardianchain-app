import { CapsuleData, ExtendedCapsuleData, CapsuleCreationRequest, CapsuleValidation, YieldClaim } from '@shared/types/capsule';
...
export class CapsuleService {
   ...
   */
  static async createCapsule(request: CapsuleCreationRequest): Promise<ExtendedCapsuleData> {
    const capsuleId = uuidv4();
    const veritasId = this.generateVeritasId();
    const sealedAt = new Date().toISOString();
    
    // Calculate grief score using AI if not provided
    const griefScore = request.expectedGriefScore || await this.calculateGriefScore(request.content);
    
    // Calculate yield amount based on grief score
    const yieldAmount = this.calculateYieldAmount(griefScore);
    
    // Generate content hash for integrity verification
    const proofHash = this.generateProofHash(request.content, request.authorWalletAddress, griefScore);
    ...
    const capsule: ExtendedCapsuleData = {
      ...
      mediaAttachments: request.mediaFiles?.map(file => ({
        type: file.type,
  ...
  private static async calculateGriefScore(content: string): Promise<number> {
    ...
    if (words.includes('death') || words.includes('loss') || words.includes('grief')) score += 20;
    if (words.includes('love') || words.includes('family') || words.includes('remember')) score += 15;
    if (words.includes('injustice') || words.includes('wrong') || words.includes('truth')) score += 10;
    if (words.includes('hope') || words.includes('future') || words.includes('change')) score += 5;
  ...
  private static calculateYieldAmount(griefScore: number): number {
    ...
    return Math.round(baseYield * griefMultiplier * bonusMultiplier * 100) / 100;
  }
  
  /**
   * Generates cryptographic proof hash
   */
  private static generateProofHash(content: string, author: string, griefScore: number): string {
