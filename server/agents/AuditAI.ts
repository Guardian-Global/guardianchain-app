...
export class AuditAI {
  ...
  static async runAudit(data: {
    ...
    const report: AuditReport = {
      ...
      pendingInvoices: data.invoices.filter((inv) => inv.status === "pending")
        .length,
...
- **Invoice Processing:** ${(
      (data.invoices.filter((i: InvoiceData) => i.status === "paid").length /
