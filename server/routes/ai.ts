...
Respond with JSON in this exact format:
{
  ...
  "recommendedActions": ["action1", "action2"]
          ...
          messages: [
            {
              role: "system",
              content:
                "You are an expert content analyst for a truth preservation platform. Provide accurate, helpful analysis in valid JSON format.",
            },
            {
              role: "user",
              content: prompt,
        ...
        console.log(
          `🧠 AI analysis completed for capsule ${capsuleId} using OpenAI`,
        );
      } catch (openaiError) {
        console.warn(
          "⚠️ OpenAI analysis failed, using mock data:",
          openaiError,
        );
  ...
  try {
    ...
    if (openai && text.length > 0) {
      try {
        ...
        console.log(
          `🌐 Translation completed: ${sourceLanguage} → ${targetLanguage} using OpenAI`,
        );
      } catch (openaiError) {
