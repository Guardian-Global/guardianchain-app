...
router.get('/api/compliance/score', async (req: Request, res: Response) => {
  ...
  const categoryScores = Object.values(complianceMetrics).map(category => 
    Object.values(category).reduce((sum, score) => sum + score, 0) / Object.values(category).length
  );
...
router.get('/api/compliance/readiness', async (req: Request, res: Response) => {
  ...
  const passedChecks = Object.values(checks).reduce((sum, category) => 
  ...
  res.json({
    ...
    blockers: completionPercentage < 100 ? ['Mumbai Testnet requires MATIC funding'] : [],
    timestamp: new Date().toISOString()
  });
});
