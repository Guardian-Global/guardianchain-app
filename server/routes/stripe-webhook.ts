...
export function registerStripeWebhook(app: Express) {
  app.post('/api/stripe/webhook', async (req, res) => {
    if (!stripe) {
      console.log('Stripe webhook received but no Stripe secret key configured');
    ...
    try {
      ...
      if (event.type === 'customer.subscription.created' || 
        ...
        if (userId && tier) {
          ...
          console.log(`Updated user ${userId} to ${tierName} tier with ${tier.capsuleMints} credits`);
        }
      }
      ...
      if (event.type === 'customer.subscription.deleted') {
        ...
        if (userId) {
          ...
          console.log(`User ${userId} subscription cancelled, reset to Explorer tier`);
        }
      }
