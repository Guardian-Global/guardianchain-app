...
router.get('/launch-status', isAuthenticated, async (req, res) => {
  try {
    ...
    for (const network of networks) {
      try {
        const deploymentFile = path.join(deploymentsDir, `${network.name}_gtt.json`);
        ...
        if (stats.isFile()) {
          ...
          try {
            ...
            const networkLiquidityFile = liquidityFiles.find(f => f.startsWith(network.name));
            
            if (networkLiquidityFile) {
              const liquidityData = JSON.parse(
                await fs.readFile(path.join(liquidityDir, networkLiquidityFile), 'utf8')
              );
    ...
    try {
      const applicationsDir = path.join(deploymentsDir, 'exchange-applications');
      ...
      for (const exchange of exchanges) {
        ...
        if (applicationFile) {
          const application = JSON.parse(
            await fs.readFile(path.join(applicationsDir, applicationFile), 'utf8')
          );
    ...
    const deployedNetworks = networks.filter(n => n.status === 'deployed').length;
    const appliedExchanges = exchanges.filter(e => e.status !== 'not_applied').length;
    const configuredBridges = bridges.filter(b => b.status !== 'not_configured').length;
    
    const totalTasks = networks.length + exchanges.length + bridges.length;
    const completedTasks = deployedNetworks + appliedExchanges + configuredBridges;
...
router.post('/add-liquidity', isAuthenticated, async (req, res) => {
  try {
    ...
    if (!network || !dex || !amounts) {
      return res.status(400).json({ error: 'Network, DEX, and amounts are required' });
    }
...
router.post('/submit-exchange-application', isAuthenticated, async (req, res) => {
  try {
    ...
    const scriptPath = path.join(process.cwd(), 'scripts', `apply${exchange}.js`);
...
router.post('/configure-bridge', isAuthenticated, async (req, res) => {
  try {
    ...
    console.log(`Configuring ${bridge} bridge: ${sourceNetwork} → ${targetNetwork}...`);
...
function generateNextActions(networks: any[], exchanges: any[], bridges: any[]): string[] {
  const actions: string[] = [];
  
  const pendingNetworks = networks.filter(n => n.status === 'pending');
  const deployedNetworks = networks.filter(n => n.status === 'deployed');
  const unappliedExchanges = exchanges.filter(e => e.status === 'not_applied');
  const unconfiguredBridges = bridges.filter(b => b.status === 'not_configured');
  
  if (pendingNetworks.length > 0) {
    actions.push(`Deploy GTT to ${pendingNetworks.length} remaining networks`);
  }
  
  if (deployedNetworks.length > 0 && deployedNetworks.some(n => n.liquidityPools.length === 0)) {
    actions.push('Add initial liquidity to deployed networks');
  }
  
  if (unappliedExchanges.length > 0) {
    actions.push(`Submit listing applications to ${unappliedExchanges.length} exchanges`);
  }
