...
router.get('/tier/:tierName', (req: Request, res: Response) => {
  ...
  const tier = PRICING_MATRIX.tiers.find(t => 
  ...
  res.json({
    ...
    data: {
      tier,
      features,
      monthlyPrice: PRICING_MATRIX.stripeMonthlyPrices.find(p => p.tier === tier.name.toUpperCase().replace(' ', '_')),
      annualPrice: PRICING_MATRIX.stripeAnnualPrices.find(p => p.tier === tier.name.toUpperCase().replace(' ', '_'))
    }
  });
});
 ...
 */
router.post('/calculate-upgrade', authenticateToken, async (req: Request, res: Response) => {
  try {
    ...
    const currentTier = PRICING_MATRIX.tiers.find(t => t.id === req.user!.tier.toLowerCase());
    const target = PRICING_MATRIX.tiers.find(t => 
    ...
    const priceList = billingInterval === 'year' ? 
      PRICING_MATRIX.stripeAnnualPrices : 
      PRICING_MATRIX.stripeMonthlyPrices;

    const targetPrice = priceList.find(p => p.tier === target.name.toUpperCase().replace(' ', '_'));
    const currentPrice = priceList.find(p => p.tier === currentTier.name.toUpperCase().replace(' ', '_'));

    const upgradeCost = targetPrice ? targetPrice.amount : target.price * 100;
    const currentCost = currentPrice ? currentPrice.amount : currentTier.price * 100;
    ...
    res.json({
      ...
      data: {
        ...
        billingInterval,
        savings: billingInterval === 'year' ? Math.round((target.price * 2) * 100) : 0,
        features: PRICING_MATRIX.features.filter(f => 
          f.tiers.includes(target.name.toUpperCase().replace(' ', '_')) &&
          !f.tiers.includes(currentTier.name.toUpperCase().replace(' ', '_'))
        )
      }
    });
...
router.get('/validate', (req: Request, res: Response) => {
  ...
  PRICING_MATRIX.tiers.forEach(tier => {
    const monthlyPrice = PRICING_MATRIX.stripeMonthlyPrices.find(p => 
      p.tier === tier.name.toUpperCase().replace(' ', '_')
    );
    
    if (tier.price > 0 && (!monthlyPrice || monthlyPrice.amount !== tier.price * 100)) {
      errors.push(`Price mismatch for ${tier.name}: Expected ${tier.price * 100}, got ${monthlyPrice?.amount || 'none'}`);
    }
  });

  // Check for placeholder price IDs
  [...PRICING_MATRIX.stripeMonthlyPrices, ...PRICING_MATRIX.stripeAnnualPrices].forEach(price => {
    if (price.id.includes('placeholder')) {
      errors.push(`Stripe price ID for ${price.tier} ${price.interval} is still a placeholder`);
    }
  });
