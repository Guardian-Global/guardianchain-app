  ...
  try {
    ...
    const completion = await openai.chat.completions.create({
      ...
      messages: [
        {
          role: "system",
          content: "You are Veritus, an AI engine specialized in content analysis for the GUARDIANCHAIN protocol. Provide accurate, actionable insights about content truth, originality, viral potential, and monetization value. Always respond with valid JSON."
        },
        {
          role: "user",
          content: prompt
    ...
    const validatedAnalysis = {
      ...
      originalityScore: Math.min(100, Math.max(0, analysis.originalityScore || 50)),
      ...
      risks: Array.isArray(analysis.risks) ? analysis.risks.slice(0, 4) : [
        'Content may be vulnerable to plagiarism',
        'Unverified claims require fact-checking'
      ],
      evidence: Array.isArray(analysis.evidence) ? analysis.evidence.slice(0, 5) : [
...
`;
    ...
    const completion = await openai.chat.completions.create({
      ...
      messages: [
        {
          role: "system",
          content: "You are a content valuation expert. Provide realistic market-based valuations for digital content."
        },
        {
          role: "user",
          content: prompt
    ...
    res.json({
      ...
      factors: valuation.factors || ['Content quality', 'Audience size', 'Market demand'],
      monetizationMethods: valuation.monetizationMethods || ['Sponsorships', 'Affiliate marketing', 'Digital products']
    });
    ...
    res.json({
      ...
      monetizationMethods: ['Direct sponsorships', 'Content licensing', 'Audience building']
    });
  }
});
...
`;
    ...
    const completion = await openai.chat.completions.create({
      ...
      messages: [
        {
          role: "system",
          content: "You are a viral content expert specializing in social media optimization and engagement prediction."
        },
        {
          role: "user",
          content: prompt
    ...
    res.json({
      ...
      factors: prediction.factors || ['Engagement potential', 'Content quality'],
      timing: prediction.optimalTiming || '2:00 PM EST (Peak engagement)',
      hashtags: prediction.suggestedHashtags || ['#viral', '#trending', '#content'],
      improvements: prediction.improvements || ['Add call-to-action', 'Include visual elements']
    });
