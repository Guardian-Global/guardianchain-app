      ...
      messages: [
        {
          role: "system",
          content:
            "You are a blockchain enterprise consultant analyzing potential value creation from GUARDIANCHAIN truth verification protocol implementation.",
        },
        {
          role: "user",
...
router.post("/enterprise-roi", async (req, res) => {
  try {
    ...
    const roiPercentage = (
      (netAnnualSavings / annualGuardianChainCost) *
      100
    ).toFixed(1);
    ...
    res.json({
      ...
      recommendations: [
        netAnnualSavings > 500000
          ? "Enterprise Alliance tier recommended"
          : "Protocol Sovereign tier optimal",
