...
export function registerTierRoutes(app: Express) {
  ...
  // Update GTT balance
  app.put('/api/users/:userId/gtt-balance', async (req: Request, res: Response) => {
    try {
      const { userId } = req.params;
  ...
  app.get('/api/users/:userId/access', async (req: Request, res: Response) => {
    try {
      ...
      const access = {
        ...
        remainingMints: Math.max(getTierMintLimit(user.tierId) - user.mintsThisPeriod, 0),
  ...
  app.post('/api/stripe/create-session', async (req: Request, res: Response) => {
    try {
      const { tierId, userId, billingPeriod } = TierUpgradeSchema.parse(req.body);
...
function getPriceIdForTier(tierId: string, billingPeriod: string): string {
  ...
  const priceIds = {
    'seeker': billingPeriod === 'annual' ? 'price_seeker_annual' : 'price_seeker_monthly',
    'creator': billingPeriod === 'annual' ? 'price_creator_annual' : 'price_creator_monthly',
    'sovereign': billingPeriod === 'annual' ? 'price_sovereign_annual' : 'price_sovereign_monthly'
  };
