...
export async function getCapsuleAnalytics(req: Request, res: Response) {
  try {
    ...
    const generateAnalyticsData = (id: string, range: string): YieldDataPoint[] => {
    ...
    const totalYield = dataPoints.reduce((sum, point) => sum + point.yieldAmount, 0);
    const avgResonance = dataPoints.reduce((sum, point) => sum + point.emotionalResonance, 0) / dataPoints.length;
    ...
    res.json({
      ...
      metadata: {
        ...
        totalVerifications: dataPoints.reduce((sum, dp) => sum + dp.verificationCount, 0),
