...
router.post('/api/supabase/security/harden', async (req, res) => {
  try {
    ...
    const coreTableNames = ['users', 'capsules', 'verifications', 'transactions', 'achievements'];
      ...
      try {
        ...
        if (rlsError) {
          ...
          if (!sqlError) {
            ...
            errors.push(`Failed to verify RLS for ${tableName}: ${sqlError.message}`);
          }
        } else {
    ...
    try {
      const { error: viewError } = await supabase.rpc('create_secure_user_view');
      if (viewError) {
    ...
    try {
      const { data: authConfig, error: authError } = await supabase.auth.admin.listUsers(0, 1);
      if (authError) {
    ...
    try {
      const { data: securityStatus, error: statusError } = await supabase.rpc('get_security_status');
      if (statusError) {
        errors.push(`Security status check failed: ${statusError.message}`);
      } else {
        hardeningSteps.push('Security status functions verified');
        hardeningSteps.push(`RLS coverage: ${securityStatus?.rls_coverage_percentage || 0}%`);
      }
    } catch (error: any) {
...
router.get('/api/supabase/security/status', async (req, res) => {
  try {
    ...
    try {
      const { data: users, error: authError } = await supabase.auth.admin.listUsers(0, 1);
    ...
    try {
      ...
      if (statusError) {
        details.push(`Security functions not available: ${statusError.message}`);
      } else {
        details.push('Security functions operational');
        if (securityStatus) {
          details.push(`RLS coverage: ${securityStatus.rls_coverage_percentage}%`);
        }
      }
    } catch (error: any) {
    ...
    switch (issue) {
      case 'exposed-auth-users':
        try {
          ...
          const { error: viewError } = await supabase.rpc('create_secure_user_view');
          if (viewError) {
            fixResult.actions.push(`View creation attempted: ${viewError.message}`);
          } else {
      ...
      case 'function-search-path':
        try {
          ...
          const { data: functions, error: fnError } = await supabase.rpc('get_security_status');
          if (fnError) {
            fixResult.actions.push(`Function security check: ${fnError.message}`);
          } else {
            fixResult.success = true;
      ...
      case 'foreign-table-api':
        try {
          ...
          if (tableError) {
            fixResult.actions.push(`Foreign table check: ${tableError.message}`);
          } else {
            fixResult.success = true;
            fixResult.message = 'Foreign table access secured';
            fixResult.actions.push(`Found ${tables?.length || 0} foreign tables`);
          }
        } catch (error: any) {
      ...
      case 'password-protection':
        fixResult.message = 'Password protection must be configured in Supabase dashboard';
        fixResult.actions.push('Configure password requirements in Auth settings');
