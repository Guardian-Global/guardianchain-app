...
function determineRecipient(subject: string, priority: string): string {
  ...
  if (priority === 'emergency' || subjectLower.includes('security') || subjectLower.includes('emergency')) {
  ...
  if (subjectLower.includes('partnership') || subjectLower.includes('business')) {
...
async function sendConfirmationEmail(email: string, name: string, subject: string) {
  // In production, send confirmation email using Resend
  console.log(`Confirmation email would be sent to ${email} for subject: ${subject}`);
}