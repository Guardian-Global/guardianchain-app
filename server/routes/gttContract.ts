...
export function registerGTTContractRoutes(app: Express) {
  ...
  app.post('/api/gtt/vault/distribute', isDebugAuthenticated, async (req: any, res) => {
    try {
      ...
      if (CONTRACT_CONFIG.GTT_YIELD_VAULT_ADDRESS === '0x0000000000000000000000000000000000000000') {
        ...
        console.log('🔵 DEBUG: GTT Vault yield distribution (development mode):', simulatedResult);
      ...
      const result = await yieldVaultService.distributeYield(authorAddress, griefTier);
  ...
  app.get('/api/gtt/vault/info', isDebugAuthenticated, async (req: any, res) => {
    try {
      if (CONTRACT_CONFIG.GTT_YIELD_VAULT_ADDRESS === '0x0000000000000000000000000000000000000000') {
        return res.json({
          success: true,
  ...
  app.post('/api/gtt/vault/update-admin', isDebugAuthenticated, async (req: any, res) => {
    try {
      ...
      const transactionHash = await yieldVaultService.updateAdmin(newAdminAddress);
