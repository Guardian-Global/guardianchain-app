...
router.post('/api/airdrop/claim', async (req, res) => {
  try {
    ...
    if (!isEligible) {
      return res.status(400).json({ message: 'Address not eligible for airdrop' });
    }
...
router.post('/api/referral/generate', async (req, res) => {
  try {
    ...
    const code = `GUARD${Math.random().toString(36).substr(2, 4).toUpperCase()}`;
...
router.post('/api/referral/reward', async (req, res) => {
  try {
    ...
    if (!referralCode || !newUserAddress) {
      return res.status(400).json({ message: 'Referral code and user address are required' });
    }
