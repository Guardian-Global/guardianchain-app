...
export async function notifyDAOVote({ 
  ...
  await sendGuardianEmail({
    to: user.email,
    subject: `${typeEmojis[proposalType]} DAO Vote Confirmed - ${vote.toUpperCase()}`,
...
## Current Vote Tally
- ✅ **For:** ${currentResults.for.toLocaleString()} GTT (${((currentResults.for / (currentResults.for + currentResults.against + currentResults.abstain)) * 100).toFixed(1)}%)
- ❌ **Against:** ${currentResults.against.toLocaleString()} GTT (${((currentResults.against / (currentResults.for + currentResults.against + currentResults.abstain)) * 100).toFixed(1)}%)
- ⚖️ **Abstain:** ${currentResults.abstain.toLocaleString()} GTT (${((currentResults.abstain / (currentResults.for + currentResults.against + currentResults.abstain)) * 100).toFixed(1)}%)
...
## What Happens Next
- ⏳ **Voting Period:** ${proposalType === "emergency" ? "24 hours" : "7 days"} remaining
...
- **Governance Level:** ${votingPower > 10000 ? "Elite Voter" : votingPower > 5000 ? "Active Participant" : "Community Member"}
- **Success Rate:** ${(Math.random() * 30 + 70).toFixed(1)}% (voted with majority)
