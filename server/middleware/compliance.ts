...
export const privacyCompliance = (req: Request, res: Response, next: NextFunction) => {
  // Set privacy headers
...
export const complianceTracking = (req: Request, res: Response, next: NextFunction) => {
  // Log compliance-relevant requests
  if (req.path.includes('/api/')) {
    console.log(`[COMPLIANCE] ${new Date().toISOString()} - ${req.method} ${req.path} - IP: ${req.ip}`);
  }
...
export const dataProcessingConsent = (req: Request, res: Response, next: NextFunction) => {
  // Ensure data processing consent for relevant endpoints
  const dataProcessingEndpoints = ['/api/users', '/api/capsules', '/api/profile'];
  
  if (dataProcessingEndpoints.some(endpoint => req.path.startsWith(endpoint))) {
...
export const financialCompliance = (req: Request, res: Response, next: NextFunction) => {
