...
export function calculateTruthYield(capsule: Capsule): number {
  ...
  const griefMultiplier = Math.max(0.1, 1 - (parseFloat(griefScore || "0") * 0.1));
...
export function calculateDailyYieldRate(capsule: Capsule, daysActive: number): number {
...
export function estimateYieldProjection(
  ...
  const dailyYieldGrowth = 
    (dailyViews * ROI_WEIGHTS.VIEW) + 
    (dailyShares * ROI_WEIGHTS.SHARE);
...
export function canClaimYield(
  ...
  if (yieldAmount < 1.0) {
    return { canClaim: false, reason: "Minimum yield of 1.0 required to claim" };
  }
  
  // Check cooldown period (7 days between claims)
  if (lastClaimDate) {
    const daysSinceLastClaim = (Date.now() - lastClaimDate.getTime()) / (1000 * 60 * 60 * 24);
    if (daysSinceLastClaim < 7) {
      return { canClaim: false, reason: `Can claim again in ${Math.ceil(7 - daysSinceLastClaim)} days` };
    }
  }
