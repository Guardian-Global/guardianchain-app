import { fetchTokenData, fetchGTTPrice, checkWeb3Health } from '../lib/web3/token';
import { calculateYield, calculateCapsuleRewards, validateYieldParams } from '../lib/finance/engine';
...
describe('GTT Token Integration Tests', () => {
  ...
  describe('Token Data Fetching', () => {
    test('fetchTokenData should return valid token information', async () => {
      ...
      expect(result.contractAddress).toBe("0x742d35Cc66535C0532925a3b8d0E9B01d9c5d9A6C");
  ...
  describe('Yield Calculations', () => {
    ...
    test('validateYieldParams should catch invalid inputs', () => {
      ...
      expect(invalidAmount.errors).toContain('Staking amount must be greater than 0');
