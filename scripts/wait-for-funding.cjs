...
async function waitAndDeploy() {
  ...
  console.log("🎯 Target GTT Address: 0x742d35Cc66535C0532925a3b8d0E9B01d9c5d9A6C");
  ...
  const checkBalance = async () => {
    try {
      ...
      console.log(`⏰ ${new Date().toLocaleTimeString()} - Balance: ${balanceInMatic} MATIC`);
  ...
  const deployGTT = async () => {
    try {
      ...
      console.log("🔗 PolygonScan:", `https://polygonscan.com/address/${deployedAddress}`);
      console.log("🎯 Target Address Match:", deployedAddress === "0x742d35Cc66535C0532925a3b8d0E9B01d9c5d9A6C" ? "✅ YES" : "⚠️ Different");
      ...
      const deploymentInfo = {
        ...
        tokenDetails: { name, symbol, totalSupply: ethers.formatEther(totalSupply) }
      };

      fs.writeFileSync('gtt-mainnet-deployment-SUCCESS.json', JSON.stringify(deploymentInfo, null, 2));
