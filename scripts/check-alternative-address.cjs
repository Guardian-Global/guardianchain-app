...
async function main() {
  ...
  try {
    ...
    if (deployer.address.toLowerCase() !== screenshotAddress.toLowerCase()) {
      ...
      const screenshotBalance = await hre.ethers.provider.getBalance(screenshotAddress);
      console.log("Screenshot address balance:", hre.ethers.formatEther(screenshotBalance), "MATIC");
    }
