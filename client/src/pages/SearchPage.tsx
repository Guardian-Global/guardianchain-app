...
export default function SearchPage() {
  ...
  const { data: searchResults, isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      const response = await apiRequest('GET', `/api/search?q=${encodeURIComponent(activeSearch)}&type=${searchType}&sort=${sortBy}`);
  ...
  const getTypeIcon = (type: string) => {
    switch (type) {
      ...
      case 'guardian': return <User className="w-4 h-4" />;
      ...
      <div className="relative z-10 container mx-auto px-4 py-8">
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-xl text-gray-300 max-w-3xl mx-auto">
            Discover truth capsules, guardians, and eternal contracts across the GuardianChain ecosystem
          </p>
        </div>
                ...
                <div className="flex-1">
                  <Select value={searchType} onValueChange={setSearchType}>
                    ...
                    <SelectContent className="bg-slate-800 border-indigo-500/30">
                      ...
                      <SelectItem value="contract">Eternal Contracts</SelectItem>
                    </SelectContent>
                  </Select>
                </div>
            ...
            {isLoading ? (
              <Card className="bg-black/40 backdrop-blur-xl border-gray-500/20">
                <CardContent className="text-center py-12">
                  ...
                  <p className="text-gray-400">Searching across the truth network...</p>
                </CardContent>
              </Card>
            ) : searchResults && searchResults.length > 0 ? (
              <div className="space-y-4">
                {searchResults.map((result: SearchResult) => (
                  <Card key={result.id} className="bg-black/40 backdrop-blur-xl border-indigo-500/20 hover:bg-black/60 transition-colors">
                    <CardContent className="p-6">
                        ...
                        <div className="flex items-center space-x-3">
                          ...
                          <div>
                            ...
                            <p className="text-sm text-gray-400">
                              by {result.creator} • {new Date(result.created_at).toLocaleDateString()}
                            </p>
                          </div>
                        </div>
                        
                        <div className="flex items-center space-x-2">
                          <Badge className={`${getVerificationColor(result.verification_status)} text-white border-none`}>
                        ...
                        <div className="flex items-center space-x-4">
                          {result.grief_tier > 0 && (
                            <div className="flex items-center space-x-1">
                              <Heart className={`w-4 h-4 ${getGriefTierColor(result.grief_tier)}`} />
                              <span className={`text-sm ${getGriefTierColor(result.grief_tier)}`}>
        ...
        {!activeSearch && (
          <div className="max-w-4xl mx-auto">
            <Card className="bg-black/40 backdrop-blur-xl border-gray-500/20">
              <CardHeader>
                <CardTitle className="text-xl text-indigo-300">Popular Searches</CardTitle>
              </CardHeader>
              <CardContent>
