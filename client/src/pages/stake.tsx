...
import { Coins, TrendingUp, Award, Shield, Users, Clock, Zap } from 'lucide-react';
import { Card, CardContent, CardHeader, CardTitle } from '@/components/ui/card';
import { Button } from '@/components/ui/button';
import { Input } from '@/components/ui/input';
import { Badge } from '@/components/ui/badge';
import { useToast } from '@/hooks/use-toast';
import { BRAND_NAME, BRAND_COLORS, STAKING_TIERS, EARLY_ADOPTER_REWARDS } from '@/lib/constants';
...
export default function StakePage() {
  ...
  const getTierInfo = (tier: string) => {
    return STAKING_TIERS[tier as keyof typeof STAKING_TIERS] || STAKING_TIERS.BRONZE;
  };
            ...
            <CardContent className="p-6">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-2xl font-bold text-white">{userStats.staked.toLocaleString()}</p>
                  <p className="text-xs text-slate-500">GTT</p>
                </div>
            ...
            <CardContent className="p-6">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-2xl font-bold text-white">{userStats.rewards.toLocaleString()}</p>
                  <p className="text-xs text-slate-500">GTT</p>
                </div>
            ...
            <CardContent className="p-6">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-2xl font-bold text-white">{userStats.tier}</p>
                  <p className="text-xs text-slate-500">{currentTier.multiplier}x rewards</p>
                </div>
                <div className="p-2 bg-purple-500/20 rounded-lg">
              ...
              <CardContent className="space-y-6">
                <div>
                  ...
                  <div className="flex gap-2">
                    ...
                    <Button
                      variant="outline"
                      onClick={() => setStakeAmount(userStats.balance.toString())}
                ...
                <div className="bg-slate-700/20 rounded-lg p-4">
                  <h4 className="text-white font-semibold mb-3">Staking Benefits</h4>
                  <div className="space-y-2">
                    <div className="flex items-center justify-between text-sm">
                      <span className="text-slate-400">Current APY</span>
                      <span className="text-green-400 font-semibold">{(currentTier.multiplier * 15).toFixed(1)}%</span>
                    </div>
                    <div className="flex items-center justify-between text-sm">
                      <span className="text-slate-400">Governance Power</span>
                      <span className="text-blue-400 font-semibold">+{currentTier.multiplier}x voting weight</span>
                    </div>
                    <div className="flex items-center justify-between text-sm">
              ...
              <CardContent className="space-y-4">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="bg-yellow-900/20 border border-yellow-600/30 rounded-lg p-4">
                    <h4 className="text-yellow-400 font-semibold mb-2">First 100 Stakers</h4>
                    <p className="text-white text-xl font-bold">{EARLY_ADOPTER_REWARDS.FIRST_100_STAKERS} GTT</p>
                    <p className="text-xs text-slate-400">Bonus reward for early participation</p>
                  </div>
                  <div className="bg-green-900/20 border border-green-600/30 rounded-lg p-4">
                    <h4 className="text-green-400 font-semibold mb-2">Referral Bonus</h4>
                    <p className="text-white text-xl font-bold">{EARLY_ADOPTER_REWARDS.REFERRAL_BONUS} GTT</p>
                    <p className="text-xs text-slate-400">Per successful referral</p>
                  </div>
                </div>
              </CardContent>
            </Card>
                ...
                {Object.entries(STAKING_TIERS).map(([tier, config]) => (
                  ...
                  >
                    <div className="flex items-center justify-between mb-2">
                      ...
                      <span className="text-sm text-slate-400">{config.multiplier}x rewards</span>
                    </div>
