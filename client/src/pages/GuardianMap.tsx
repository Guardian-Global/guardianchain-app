...
export default function GuardianMap() {
  ...
  const drawConnections = (
    ...
    networkConnections.forEach((connection) => {
      ...
      const sourcePos = latLongToPixel(
        source.latitude,
        source.longitude,
        width,
        height,
      );
      const targetPos = latLongToPixel(
        target.latitude,
        target.longitude,
        width,
        height,
  ...
  const drawHeatmap = (
    ...
    guardians.forEach((guardian) => {
      ...
      // Draw heat circle
      for (
        let x = Math.max(0, pos.x - radius);
        x < Math.min(width, pos.x + radius);
        x++
      ) {
        for (
          let y = Math.max(0, pos.y - radius);
          y < Math.min(height, pos.y + radius);
          y++
        ) {
          ...
          if (distance <= radius) {
            ...
            heatmapData.data[pixelIndex] = Math.min(
              255,
              heatmapData.data[pixelIndex] + heatValue * 255,
            ); // Red
            heatmapData.data[pixelIndex + 1] = Math.min(
              255,
              heatmapData.data[pixelIndex + 1] + heatValue * 100,
            ); // Green
            heatmapData.data[pixelIndex + 2] = Math.min(
              255,
              heatmapData.data[pixelIndex + 2] + heatValue * 50,
            ); // Blue
            heatmapData.data[pixelIndex + 3] = Math.min(
              255,
              heatmapData.data[pixelIndex + 3] + heatValue * 128,
            ); // Alpha
          }
        }
      }
  ...
  const drawGuardians = (
    ...
    guardians.forEach((guardian) => {
      ...
      const radius = Math.max(
        3,
        Math.min(15, (guardian.truth_score / 10) * (zoomLevel[0] / 50)),
      );
  ...
  const handleFiltersChange = (filters: any) => {
    ...
    let filtered = mapData.guardians.filter((guardian: GuardianNode) => {
      ...
      if (filters.searchQuery) {
        ...
        const matches =
          guardian.wallet.toLowerCase().includes(query) ||
      ...
      // Region filter
      if (filters.region !== "all" && guardian.region !== filters.region)
        return false;

      // Tier filter
      if (filters.tier !== "all" && guardian.reputation_tier !== filters.tier)
        return false;

      // Activity level filter
      if (
        filters.activityLevel !== "all" &&
        guardian.activity_level !== filters.activityLevel
      )
        return false;

      // Truth score range
      if (
        guardian.truth_score < filters.truthScoreRange[0] ||
        guardian.truth_score > filters.truthScoreRange[1]
      )
        return false;

      // Capsule count range
      if (
        guardian.capsule_count < filters.capsuleCountRange[0] ||
        guardian.capsule_count > filters.capsuleCountRange[1]
      )
        return false;
      ...
      if (filters.hasActiveConnections && guardian.connections.length === 0)
        return false;
  ...
  const handleExportData = (format: "csv" | "json" | "excel", options: any) => {
    const exportData = filteredGuardians.map((guardian) => ({
      ...
      ...(options.includeSpecialties && {
        specialties: guardian.specialties.join(", "),
      }),
      ...(options.includeConnections && {
        connections: guardian.connections.join(", "),
      }),
  ...
  const selectedGuardian = filteredGuardians.find(
    (g: GuardianNode) => g.id === selectedNode,
  );
      ...
      <div className="relative z-10 container mx-auto px-4 py-8">
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-xl text-gray-300 max-w-3xl mx-auto">
            Explore the global network of truth guardians and their
                  ...
                  {selectedGuardian.specialties.length > 0 && (
                    <div>
                      ...
                      <div className="flex flex-wrap gap-1">
                        {selectedGuardian.specialties.map(
                          (specialty, index) => (
                            <Badge
                              key={index}
                              variant="outline"
