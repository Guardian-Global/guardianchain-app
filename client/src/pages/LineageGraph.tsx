...
export default function LineageGraph() {
  ...
  const [viewMode, setViewMode] = useState<'tree' | 'force' | 'timeline'>('tree');
  ...
  useEffect(() => {
    ...
    const positionedNodes = calculateLayout(lineageData.nodes, lineageData.connections, viewMode, width, height);
    
    renderGraph(svg, positionedNodes, lineageData.connections, width, height);
  }, [lineageData, viewMode]);

  const calculateLayout = (nodes: LineageNode[], connections: LineageConnection[], mode: string, width: number, height: number) => {
    switch (mode) {
      case 'tree':
  ...
  const calculateTreeLayout = (nodes: LineageNode[], connections: LineageConnection[], width: number, height: number) => {
  ...
  const calculateForceLayout = (nodes: LineageNode[], connections: LineageConnection[], width: number, height: number) => {
  ...
  const calculateTimelineLayout = (nodes: LineageNode[], width: number, height: number) => {
  ...
  const renderGraph = (svg: SVGSVGElement, nodes: LineageNode[], connections: LineageConnection[], width: number, height: number) => {
  ...
  const selectedNodeData = lineageData?.nodes?.find((n: LineageNode) => n.id === selectedNode);
      ...
      <div className="relative z-10 container mx-auto px-4 py-8">
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-xl text-gray-300 max-w-3xl mx-auto">
            Explore the interconnected web of truth capsules and their influence on the Guardian ecosystem
          </p>
        </div>
              ...
              <div className="flex flex-wrap gap-4 items-center">
                ...
                <div className="flex space-x-2">
                  {['tree', 'force', 'timeline'].map((mode) => (
                    ...
                    >
                      {mode === 'tree' && <GitBranch className="w-4 h-4 mr-1" />}
                      {mode === 'force' && <Zap className="w-4 h-4 mr-1" />}
                      {mode === 'timeline' && <Clock className="w-4 h-4 mr-1" />}
        ...
        <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-4 gap-6">
          ...
          <div className="space-y-4">
            ...
            <Card className="bg-black/40 backdrop-blur-xl border-indigo-500/20">
              <CardHeader>
                <CardTitle className="text-indigo-300 text-sm">Network Stats</CardTitle>
              </CardHeader>
              <CardContent className="space-y-3">
                <div className="flex justify-between">
                  <span className="text-gray-400 text-sm">Total Nodes</span>
                  <span className="text-white font-semibold">{lineageData?.nodes?.length || 0}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-gray-400 text-sm">Connections</span>
                  <span className="text-white font-semibold">{lineageData?.connections?.length || 0}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-gray-400 text-sm">Verified</span>
                  <span className="text-green-400 font-semibold">
                    {lineageData?.nodes?.filter((n: LineageNode) => n.verification_status === 'verified').length || 0}
                  </span>
                </div>
              </CardContent>
            </Card>
            ...
            {selectedNodeData && (
              <Card className="bg-black/40 backdrop-blur-xl border-indigo-500/20">
                <CardHeader>
                  <CardTitle className="text-indigo-300 text-sm">Selected Capsule</CardTitle>
                </CardHeader>
                <CardContent className="space-y-4">
                  <div>
                    <h4 className="text-white font-semibold mb-2">{selectedNodeData.title}</h4>
                    <p className="text-gray-400 text-sm">by {selectedNodeData.creator}</p>
                  </div>
                  
                  <div className="space-y-2">
                    <Badge className={`${getNodeColor(selectedNodeData)} text-white border-none`}>
                  ...
                  <div className="space-y-2">
                    <div className="flex items-center space-x-2">
                      ...
                      <span className="text-sm text-gray-300">Grief Score: {selectedNodeData.grief_score}</span>
                    </div>
                    <div className="flex items-center space-x-2">
                      <TrendingUp className="w-4 h-4 text-blue-400" />
                      <span className="text-sm text-gray-300">Influences: {selectedNodeData.influence_count}</span>
                    </div>
                    <div className="flex items-center space-x-2">
                      <Clock className="w-4 h-4 text-gray-400" />
                      <span className="text-sm text-gray-300">
                        {new Date(selectedNodeData.created_at).toLocaleDateString()}
                      </span>
                    </div>
                  </div>
            ...
            <Card className="bg-black/40 backdrop-blur-xl border-indigo-500/20">
              <CardHeader>
                <CardTitle className="text-indigo-300 text-sm">Export</CardTitle>
              </CardHeader>
              <CardContent>
                <Button size="sm" variant="outline" className="w-full border-indigo-500/30 text-indigo-300">
