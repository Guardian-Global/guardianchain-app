...
export default function FlashTrading() {
  ...
  const multiplier = timeLeft > 1800 ? 15 : timeLeft > 900 ? 25 : timeLeft > 300 ? 50 : 100;
        ...
        <div className="text-center mb-8">
          ...
          <div className="bg-red-600/20 border border-red-500 rounded-lg p-4 mb-6">
            ...
            <div className="text-red-200">
              This flash trading window expires FOREVER in {formatTime(timeLeft)}
            </div>
          </div>
        </div>
            ...
            <CardContent className="space-y-6">
              <div className="bg-red-600/20 p-4 rounded-lg">
                <div className="text-red-300 text-sm mb-2">
                  ⚠️ URGENT: {multiplier}X multiplier active for {formatTime(timeLeft)}
                </div>
                <div className="text-white">
                  Every $1 traded = {multiplier} GTT tokens + {multiplier}% revenue share
                </div>
              </div>
              ...
              <div className="space-y-2">
                <div className="flex justify-between text-sm">
                  ...
                  <span className="text-green-400 font-bold">
                    {userAmount ? (parseFloat(userAmount) * multiplier).toLocaleString() : '0'} GTT
                  </span>
                </div>
                <div className="flex justify-between text-sm">
                ...
                <div className="flex justify-between text-sm">
                  ...
                  <span className="text-yellow-400 font-bold">
                    ${userAmount ? (parseFloat(userAmount) * 0.1 * multiplier / 10).toFixed(2) : '0'} instant
                  </span>
                </div>
              </div>
          ...
          <Card className="bg-gradient-to-br from-orange-900/50 to-black border-orange-500">
            ...
            <CardContent>
              <div className="space-y-3">
                {[
                  { rank: 1, address: "0x742d...9A6C", volume: 8750, reward: "$10,000" },
                  { rank: 2, address: "0x959C...39db", volume: 6200, reward: "$5,000" },
                  { rank: 3, address: "0x8c7C...0a73", volume: 4800, reward: "$2,500" },
                  { rank: 4, address: "0x1234...5678", volume: 3500, reward: "$1,000" },
                  { rank: 5, address: "0x9876...4321", volume: 2900, reward: "$500" }
                ].map((trader) => (
                  <div key={trader.rank} className="flex items-center justify-between p-3 bg-orange-600/10 rounded-lg">
        ...
        <Card className="bg-gradient-to-r from-red-600/10 to-black border-red-500">
          <CardContent className="pt-6 text-center">
            ...
            <p className="text-xl text-white mb-6">
              Flash trading windows occur RANDOMLY and last only 1 hour. 
              The next one could be weeks away. Don't miss {multiplier}X multipliers!
            </p>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
              <div className="bg-red-600/20 p-4 rounded-lg">
                <Clock className="w-8 h-8 text-red-400 mx-auto mb-2" />
                <div className="text-white font-bold">EXPIRES IN</div>
                <div className="text-2xl text-red-300">{formatTime(timeLeft)}</div>
              </div>
              <div className="bg-orange-600/20 p-4 rounded-lg">
                <Target className="w-8 h-8 text-orange-400 mx-auto mb-2" />
                <div className="text-white font-bold">CURRENT MULTIPLIER</div>
                <div className="text-2xl text-orange-300">{multiplier}X ACTIVE</div>
              </div>
