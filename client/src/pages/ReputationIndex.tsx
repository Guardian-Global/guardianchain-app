...
export default function ReputationIndex() {
  ...
  const { data: leaderboard, isLoading } = useQuery({
    ...
    queryFn: async () => {
      const response = await apiRequest(
        "GET",
        `/api/smri/leaderboard?tier=${selectedTier}`,
      );
  ...
  const { data: userSMRI } = useQuery({
    ...
    queryFn: async () => {
      const response = await apiRequest(
        "GET",
        "/api/smri/debug@guardianchain.app",
      );
      return response.json();
    },
  });
  ...
  const filteredLeaderboard =
    leaderboard?.filter((profile: SMRIProfile) =>
      ...
      <div className="relative z-10 container mx-auto px-4 py-8">
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-xl text-gray-300 max-w-3xl mx-auto">
            Track truth-telling reputation across the GuardianChain ecosystem
            with SMRI scoring
          </p>
        </div>
            ...
            <CardContent className="p-4">
              <div className="flex flex-wrap gap-4 items-center justify-between">
                ...
                <div className="flex space-x-2">
                  {["all", "Veritas", "Gold", "Silver", "Bronze"].map(
                    (tier) => (
                      <Button
                        key={tier}
              ...
              {isLoading ? (
                ...
                <div className="space-y-4">
                  {filteredLeaderboard.map(
                    (profile: SMRIProfile, index: number) => (
                      <div
                        key={profile.wallet}
                      ...
                      >
                        <div className="flex items-center space-x-4">
                          ...
                          <div>
                            ...
                            <div className="flex items-center space-x-2">
                              <Badge
                                className={`bg-gradient-to-r ${getTierColor(profile.reputation_tier)} text-white border-none text-xs`}
                              >
            ...
            <CardContent>
              <div className="bg-slate-800/50 p-4 rounded-lg border border-slate-700/50">
                <code className="text-cyan-400 text-lg">
                  Truth Score = (Grief Total + Influence × 2 + Certificates × 3)
                  × 1.5
                </code>
              </div>
              <div className="mt-4 grid grid-cols-1 md:grid-cols-4 gap-4 text-sm">
                <div className="text-center">
                  ...
                  <div className="text-gray-400">
                    Emotional impact of your truth capsules
                  </div>
                </div>
                <div className="text-center">
                  ...
                  <div className="text-gray-400">
                    How many others you've inspired
                  </div>
                </div>
                <div className="text-center">
