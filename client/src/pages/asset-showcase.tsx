...
import { Image, Video, Shield, Star, Palette, FileText, Loader2, AlertCircle } from "lucide-react";

export default function AssetShowcase() {
  const { assets, logos, videos, nftIcons, capsuleArt, loading, error, refreshAssets } = useAssets();
        ...
        <div className="container mx-auto px-4 py-8">
          <div className="flex items-center justify-center min-h-[60vh]">
            <div className="text-center">
              ...
              <h2 className="text-2xl font-bold text-white mb-2">Loading Your Assets</h2>
              <p className="text-slate-300">Discovering assets from your Supabase storage...</p>
            </div>
          </div>
        </div>
      </div>
        ...
        <div className="container mx-auto px-4 py-8">
          <div className="flex items-center justify-center min-h-[60vh]">
            <div className="text-center">
              ...
              <h2 className="text-2xl font-bold text-white mb-2">Asset Loading Error</h2>
              <p className="text-slate-300 mb-4">{error}</p>
              <Button onClick={refreshAssets} className="bg-purple-600 hover:bg-purple-700">
      ...
      <div className="container mx-auto px-4 py-8">
        ...
        <div className="text-center mb-12">
          ...
          <p className="text-xl text-slate-300 mb-6">
            Your complete collection of logos, videos, NFT art, and capsule designs from Supabase
          </p>
            ...
            {['main', 'icon', 'text', 'full'].map((variant) => (
              <Card key={variant} className="bg-slate-800/50 border-slate-700">
                <CardContent className="p-6 text-center">
                  ...
                  />
                  <p className="text-sm text-slate-300 capitalize">{variant} Logo</p>
                </CardContent>
              </Card>
            ))}
          </div>
        ...
        <section className="mb-12">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {['explainer', 'demo', 'hero', 'main'].map((variant) => (
              <Card key={variant} className="bg-slate-800/50 border-slate-700">
                <CardHeader>
                  <CardTitle className="text-white capitalize">{variant} Video</CardTitle>
                </CardHeader>
                <CardContent>
                  <VideoDisplay 
                    variant={variant as any}
        ...
        <section className="mb-12">
          <div className="flex items-center mb-6">
            ...
            <h2 className="text-2xl font-bold text-white">NFT Icons Collection</h2>
          </div>
          <div className="grid grid-cols-3 md:grid-cols-6 lg:grid-cols-10 gap-4">
            {['guardian', 'capsule', 'badge', 'tier', 'achievement'].map((type) => 
              ['common', 'rare', 'epic', 'legendary'].map((rarity) => (
                <Card key={`${type}-${rarity}`} className="bg-slate-800/50 border-slate-700 p-3">
                  <CardContent className="p-0 text-center">
                    ...
                    <p className="text-xs text-slate-500 capitalize">{rarity}</p>
                  </CardContent>
                </Card>
              ))
            )}
        ...
        <section className="mb-12">
          <div className="flex items-center mb-6">
            ...
            <h2 className="text-2xl font-bold text-white">Capsule Art Gallery</h2>
          </div>
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
            {['truth', 'legal', 'news', 'science', 'creative', 'civic'].map((category) => (
          ...
          <Card className="bg-slate-800/50 border-slate-700">
            ...
            <CardContent>
              <div className="grid grid-cols-2 md:grid-cols-4 gap-6 text-center">
                ...
                <div>
                  <div className="text-3xl font-bold text-purple-400">{logos.length}</div>
                ...
                <div>
                  <div className="text-3xl font-bold text-blue-400">{nftIcons.length + capsuleArt.length}</div>
