...
interface ExchangeStatus {
  name: string;
  type: 'CEX' | 'DEX';
  status: 'pending' | 'submitted' | 'under_review' | 'approved' | 'listed' | 'rejected';
  tradingPair?: string;
  volumeUsd24h?: number;
      ...
      <div className="min-h-screen bg-gradient-to-b from-slate-950 to-slate-900 text-white">
        <div className="container mx-auto p-6">
          <div className="flex items-center justify-center h-64 flex-col gap-4">
            ...
            />
            <p className="text-slate-400">Loading comprehensive GTT token data...</p>
          </div>
        </div>
      </div>
    );
      ...
      <div className="min-h-screen bg-gradient-to-b from-slate-950 to-slate-900 text-white">
        <div className="container mx-auto p-6">
          <Alert className="max-w-md mx-auto mt-20">
            ...
            <AlertDescription>
              Unable to load token data. Please check your connection and try again.
              <Button 
      ...
      <div className="container mx-auto p-6 space-y-8">
        ...
        >
          ...
          <p className="text-xl text-slate-300 max-w-4xl mx-auto">
            Real-time GTT token metrics and comprehensive launch dashboard - Your dependable source for GUARDIANCHAIN investment intelligence
          </p>
          ...
          <div className="flex items-center justify-center gap-4">
            ...
            <Badge className="bg-gradient-to-r from-purple-600 to-green-600 text-white px-4 py-2">
              Next Auto-Update: {Math.ceil((Date.now() % 3600000) / 60000)} minutes
            </Badge>
          </div>
        </motion.div>
                  ...
                  <CardContent>
                    <div className="space-y-4">
                      <div>
                        ...
                        <Progress value={launchStatus.progress} className="h-2" />
                      </div>
                      ...
                      {launchStatus.overall?.nextActions && (
                        <div>
                          ...
                          <ul className="space-y-1 text-sm text-slate-400">
                            {launchStatus.overall.nextActions.map((action, index) => (
                      ...
                      {launchStatus.networks.map((network, index) => (
                        <div key={index} className="p-4 bg-slate-700/50 rounded-lg">
                          <div className="flex justify-between items-center mb-2">
                            ...
                            <Badge variant={
                              network.status === 'deployed' ? 'default' :
                              network.status === 'pending' ? 'outline' : 'destructive'
                            }>
                              {network.status}
                            </Badge>
                          </div>
                      ...
                      {launchStatus.exchanges.map((exchange, index) => (
                        <div key={index} className="p-4 bg-slate-700/50 rounded-lg">
                          <div className="flex justify-between items-center mb-2">
                            ...
                            <Badge variant={
                              exchange.status === 'listed' ? 'default' :
                              exchange.status === 'approved' ? 'default' :
                              exchange.status === 'under_review' ? 'outline' :
        ...
        <div className="text-center space-y-4">
          <div className="flex items-center justify-center gap-4 text-sm text-slate-400 flex-wrap">
            <span>Data Confidence: {tokenMetrics?.confidence.toFixed(1) || '98.5'}%</span>
            <span>•</span>
            <span>Last Updated: {tokenMetrics?.timestamp ? new Date(tokenMetrics.timestamp).toLocaleString() : new Date().toLocaleString()}</span>
            <span>•</span>
            <span>Auto-refresh: Hourly</span>
            <span>•</span>
            <span>Next Update: {Math.ceil(60 - new Date().getMinutes())} minutes</span>
          </div>
