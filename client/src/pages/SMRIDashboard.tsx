...
export default function SMRIDashboard() {
  ...
  const { data: leaderboard } = useQuery({
    ...
    queryFn: async () => {
      const response = await apiRequest(
        "GET",
        `/api/smri/leaderboard?timeframe=${timeframe}`,
      );
      return response.json();
    },
  ...
  const getRankIcon = (rank: number) => {
    ...
    return (
      <div className="w-6 h-6 rounded-full bg-slate-600 flex items-center justify-center text-white text-sm font-bold">
        {rank}
      </div>
    );
  };
      ...
      <div className="relative z-10 container mx-auto px-4 py-8">
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-xl text-gray-300">
            Track your reputation journey and compete with fellow truth
            guardians
          </p>
        </div>
              ...
              <Card className="bg-black/40 backdrop-blur-xl border-purple-500/20 mb-6">
                <CardHeader>
                  <CardTitle className="text-2xl text-purple-300 flex items-center justify-between">
                    ...
                    <Badge
                      className={`bg-gradient-to-r ${getTierColor(userSMRI.reputation_tier)} text-white border-none`}
                    >
                ...
                <CardContent className="space-y-6">
                  ...
                  <div className="text-center">
                    <div
                      className={`text-6xl font-bold bg-gradient-to-r ${getTierColor(userSMRI.reputation_tier)} bg-clip-text text-transparent mb-2`}
                    >
                    ...
                    {userSMRI.trending && (
                      <div className="flex items-center justify-center mt-2">
                        <TrendingUp
                          className={`w-4 h-4 mr-1 ${userSMRI.trending === "up" ? "text-green-400" : "text-gray-400"}`}
                        />
                    ...
                    <div className="flex items-center justify-between">
                      <div>
                        ...
                        <p className="text-gray-300 text-sm">
                          {userSMRI.reputation_tier === "Bronze"
                            ? "Reach 50 points for Silver tier"
                            : userSMRI.reputation_tier === "Silver"
                              ? "Reach 100 points for Gold tier"
                              : userSMRI.reputation_tier === "Gold"
                                ? "Reach 150 points for Veritas tier"
                ...
                {leaderboard ? (
                  <div className="space-y-3">
                    {leaderboard.slice(0, 10).map((entry: LeaderboardEntry) => (
                      <div
                        key={entry.wallet}
                        className="flex items-center space-x-3 p-2 rounded-lg bg-slate-800/30"
                      >
                        ...
                        <div className="flex-1 min-w-0">
                          <p className="text-white text-sm font-medium truncate">
                            {entry.wallet.includes("@")
                              ? entry.wallet.split("@")[0]
            ...
            <Card className="bg-black/40 backdrop-blur-xl border-green-500/20 mt-6">
              ...
              <CardContent className="space-y-3">
                <div className="bg-green-900/20 rounded-lg p-3 border border-green-500/30">
                  ...
                  <Progress
                    value={userSMRI ? (userSMRI.capsule_count / 3) * 100 : 0}
                    className="h-2"
                  />
                  <p className="text-xs text-gray-400 mt-1">
                    {userSMRI ? Math.min(userSMRI.capsule_count, 3) : 0}/3
                    completed
                  </p>
                </div>
                ...
                <div className="bg-blue-900/20 rounded-lg p-3 border border-blue-500/30">
                  ...
                  <Progress
                    value={userSMRI ? (userSMRI.cert_count / 2) * 100 : 0}
                    className="h-2"
                  />
                  <p className="text-xs text-gray-400 mt-1">
                    {userSMRI ? Math.min(userSMRI.cert_count, 2) : 0}/2
                    completed
                  </p>
                </div>
                ...
                <div className="bg-purple-900/20 rounded-lg p-3 border border-purple-500/30">
                  ...
                  <Progress
                    value={
                      userSMRI
                        ? Math.min((userSMRI.influence_count / 1) * 100, 100)
                        : 0
                    }
                    className="h-2"
                  />
