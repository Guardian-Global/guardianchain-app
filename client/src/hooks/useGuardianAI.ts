...
export function useGuardianAI() {
    ...
    const insights: AIInsight[] = [
      {
        ...
        description: 'Your truth capsules could gain 23% more visibility with optimized tagging.',
        confidence: 0.89,
        impact: 'high'
      },
      {
        ...
        description: 'Based on current staking patterns, expect 15.2% APY over next 90 days.',
        confidence: 0.76,
        impact: 'medium'
      },
      {
        ...
        description: 'Consider upgrading to Veritas Sealed for premium content verification.',
        confidence: 0.92,
        impact: 'high'
      }
    ];
  ...
  const generateCapsuleTitle = async (content: string, type: string): Promise<string[]> => {
    setIsGenerating(true);
  ...
  const getRecommendations = async (userTier: string): Promise<AIRecommendation[]> => {
    setIsGenerating(true);
    ...
    const recommendations: AIRecommendation[] = [
      ...
      },
      {
        ...
        description: 'Participate in 3 more verifications to reach Truth Scholar status.',
        actionable: true
      }
    ];
