...
export async function verifyWithTruthShield(
  inputText: string,
...
function performLocalValidation(text: string): TruthShieldResponse {
  ...
  const factualClaims =
    /\d+%\s+of\s+people|studies\s+show|research\s+proves/i.test(text);
...
function generateSuggestions(issues: string[]): string[] {
  ...
  if (issues.some((issue) => issue.includes("sources"))) {
    suggestions.push(
      "Add credible sources and references to support your claims",
    );
  }
  ...
  if (suggestions.length === 0) {
    suggestions.push(
      "Content appears valid but consider adding sources for verification",
    );
  }
...
async function validateByCategory(
  text: string,
  category: string,
): Promise<string[]> {
  const issues: string[] = [];
  ...
  switch (category.toLowerCase()) {
    ...
    case "conspiracy":
      if (text.length < 100) {
        issues.push(
          "Conspiracy claims require detailed evidence and documentation",
        );
      }
      break;
  }
