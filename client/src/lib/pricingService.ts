...
class PricingService {
  ...
  getStripePriceId(tier: string, interval: "month" | "year"): string | null {
    ...
    if (interval === "month") {
      switch (tierUpper) {
        case "SEEKER":
          return STRIPE_PRICE_IDS.SEEKER_MONTHLY;
        case "CREATOR":
          return STRIPE_PRICE_IDS.CREATOR_MONTHLY;
        case "SOVEREIGN":
          return STRIPE_PRICE_IDS.SOVEREIGN_MONTHLY;
        default:
          return null;
      }
    } else {
      switch (tierUpper) {
        case "SEEKER":
          return STRIPE_PRICE_IDS.SEEKER_ANNUAL;
        case "CREATOR":
          return STRIPE_PRICE_IDS.CREATOR_ANNUAL;
        case "SOVEREIGN":
          return STRIPE_PRICE_IDS.SOVEREIGN_ANNUAL;
        default:
          return null;
      }
    }
   ...
   */
  calculateAnnualSavings(tier: string): {
    monthlyPrice: number;
    annualPrice: number;
    savings: number;
   ...
   */
  async validatePricingConfiguration(): Promise<{
    isValid: boolean;
    errors: string[];
  }> {
    const errors: string[] = [];
