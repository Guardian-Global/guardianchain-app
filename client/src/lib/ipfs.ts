...
class IPFSManager {
  ...
  async uploadFile(file: File | Buffer, filename?: string): Promise<IPFSUploadResponse> {
    try {
  ...
  private async uploadToInfura(file: File | Buffer, filename?: string): Promise<IPFSUploadResponse> {
  ...
  private async uploadToPublicGateway(file: File | Buffer, filename?: string): Promise<IPFSUploadResponse> {
  ...
  getGatewayUrl(hash: string, gateway: "ipfs" | "cloudflare" | "pinata" = "ipfs"): string {
    switch (gateway) {
      case "cloudflare":
