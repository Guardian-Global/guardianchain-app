...
export async function getAIAdvisorInsights({ treasury, market }: { treasury: any, market: any }) {
  const prompt = `
    You are the financial advisor for GuardianChain.
    Current GTT price: $${market?.price || 0}, 24h change: ${market?.change24h || 0}%
    Treasury balance: ${treasury?.balance || 0} GTT, Monthly Revenue: $${treasury?.monthlyRevenue || 0}
    Yield Paid: ${treasury?.yieldPaid || 0} GTT, Active Capsules: ${treasury?.activeCapsules || 0}
  ...
  try {
    ...
    const data = await response.json();
    return data.advice || "Analysis pending - connect OpenAI API to enable AI advisor";
  } catch (error) {
