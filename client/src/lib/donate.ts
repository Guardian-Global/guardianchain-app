...
export async function donateCapsuleCredits(donation: DonationRequest): Promise<void> {
  try {
...
export async function getDonationHistory(limit: number = 10): Promise<Donation[]> {
  try {
...
export async function donateToTraumaSurvivor(amount: number, survivorEmail: string, donorId: string) {
...
export async function donateToNonprofit(amount: number, organizationEmail: string, donorId: string, cause: string) {
  return donateCapsuleCredits({
    amount,
...
export async function donateToPublicTruth(amount: number, donorId: string, campaign: string) {
  return donateCapsuleCredits({
    amount,
...
export async function donateToCharity(amount: number, charityAddress: string, donorId: string, charityName: string) {
  return donateCapsuleCredits({
    amount,
...
export function validateDonation(donation: DonationRequest): { valid: boolean; errors: string[] } {
  const errors: string[] = [];
  ...
  if (donation.recipientType === 'individual' && donation.recipient.includes('@')) {
...
export function calculateDonationImpact(amount: number, recipientType: string): string {
