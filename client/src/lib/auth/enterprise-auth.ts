...
export const ONBOARDING_STEPS = [
  {
    ...
    aiPrompt: 'Help user understand their verification needs and select optimal tier',
  ...
  },
  {
    ...
    aiPrompt: 'Guide user through verification requirements for their jurisdiction',
...
export const userProfileSchema = z.object({
  ...
  tier: z.enum([USER_TIERS.STARTER, USER_TIERS.PROFESSIONAL, USER_TIERS.ENTERPRISE, USER_TIERS.SOVEREIGN]),
...
export const authRequestSchema = z.object({
  ...
  tier: z.enum([USER_TIERS.STARTER, USER_TIERS.PROFESSIONAL, USER_TIERS.ENTERPRISE, USER_TIERS.SOVEREIGN]).optional()
});
...
export class EnterpriseAuth {
  ...
  async authenticate(provider: string, options?: {
    tier?: UserTier;
    returnUrl?: string;
    metadata?: Record<string, any>;
  }) {
  ...
  hasPermission(user: UserProfile, permission: string): boolean {
    ...
    return tierPermissions.includes(permission) || user.permissions.includes(permission);
  }

  // Get AI onboarding recommendations
  async getOnboardingRecommendations(step: number, userContext?: Record<string, any>) {
    try {
      ...
      return {
        recommendations: ['Complete your profile setup', 'Verify your identity', 'Configure security settings'],
        nextSteps: ['Choose verification method', 'Set up two-factor authentication'],
