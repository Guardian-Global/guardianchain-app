    ...
    <Card className="bg-slate-800/50 border-slate-700">
      <CardHeader>
        ...
        <p className="text-slate-400">
          Track the complete lifecycle of a truth capsule from submission to NFT
          certification
        </p>
      </CardHeader>
      <CardContent>
              ...
              {events.map((event, index) => {
                ...
                return (
                  <div key={event.id} className="relative flex gap-6">
                    ...
                    >
                      <IconComponent
                        className={`w-5 h-5 ${getIconColor(event.status)}`}
                      />
                    </div>
                      ...
                      <div className="bg-slate-700/30 rounded-lg p-4 hover:bg-slate-700/50 transition-colors">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                          {Object.entries(event.details).map(([key, value]) => (
                            <div
                              key={key}
                              className="bg-slate-600/30 rounded p-2"
                            >
