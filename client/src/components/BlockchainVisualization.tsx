...
const BlockchainVisualization: React.FC = () => {
  ...
  const generateTransaction = (): Transaction => {
    const types: Transaction['type'][] = ['capsule_creation', 'verification', 'gtt_reward', 'transfer'];
    ...
    return new Promise((resolve) => {
      ...
      const mine = () => {
        ...
        if (hash.startsWith(`0x${target}`)) {
          const minedBlock = { ...block, nonce, hash, isMining: false, isValid: true };
          resolve(minedBlock);
        } else {
        ...
        <CardHeader>
          <CardTitle className="text-white flex items-center justify-center">
            ...
            <div className="text-center">
              <div className="text-3xl font-bold">GUARDIANCHAIN Blockchain Visualization</div>
              <div className="text-lg text-blue-400">Interactive Mining & Truth Verification Simulation</div>
            </div>
          </CardTitle>
        </CardHeader>
        <CardContent>
          ...
          <div className="grid grid-cols-2 md:grid-cols-5 gap-4 mb-6">
            ...
            <div className="bg-slate-800/50 rounded-lg p-4 text-center">
              <div className="text-2xl font-bold text-purple-400">{stats.validBlocks}</div>
              <div className="text-sm text-slate-400">Valid Blocks</div>
            </div>
            <div className="bg-slate-800/50 rounded-lg p-4 text-center">
              <div className="text-2xl font-bold text-yellow-400">{stats.hashRate}</div>
              <div className="text-sm text-slate-400">Hash Rate</div>
            </div>
            <div className="bg-slate-800/50 rounded-lg p-4 text-center">
              <div className="text-2xl font-bold text-red-400">{stats.difficulty}</div>
            ...
            <div className="space-y-4">
              <div>
                <div className="flex justify-between mb-2">
                  ...
                  <span className="text-orange-400">{miningProgress.toFixed(1)}%</span>
                </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div className="bg-slate-700/50 rounded-lg p-3">
                  ...
                  <div className="text-white font-bold">#{currentMiningBlock.id}</div>
                </div>
                <div className="bg-slate-700/50 rounded-lg p-3">
                  <div className="text-slate-400 text-sm">Transactions</div>
                  <div className="text-white font-bold">{currentMiningBlock.transactions.length}</div>
                </div>
                <div className="bg-slate-700/50 rounded-lg p-3">
                  <div className="text-slate-400 text-sm">Difficulty</div>
                  <div className="text-white font-bold">{currentMiningBlock.difficulty}</div>
                </div>
              </div>
            </div>
          </CardContent>
                  ...
                  <div key={tx.id} className="bg-slate-700/50 rounded-lg p-3">
                    <div className="flex items-center justify-between mb-2">
                      <Badge className={getTransactionColor(tx.type)}>
                        ...
                        <span className="ml-1">{tx.type.replace('_', ' ')}</span>
                      </Badge>
                    ...
                    <div className="text-sm text-slate-300">
                      ...
                      <div className="text-green-400 font-bold">{tx.amount} GTT</div>
                    </div>
                  </div>
                ))
              )}
              ...
              {blockchain.slice().reverse().map((block, index) => (
                <div key={block.id} className="relative">
                  ...
                  <div className={`rounded-lg p-4 ${
                    block.isMining ? 'bg-orange-900/20 border border-orange-700' :
                    block.isValid ? 'bg-green-900/20 border border-green-700' :
                    'bg-red-900/20 border border-red-700'
                  }`}>
                    <div className="flex items-center justify-between mb-3">
                      <div className="flex items-center">
                        <div className={`w-3 h-3 rounded-full mr-3 ${
                          block.isMining ? 'bg-orange-400 animate-pulse' :
                          block.isValid ? 'bg-green-400' : 'bg-red-400'
                        }`}></div>
                        <div>
                          <div className="text-white font-bold">Block #{block.id}</div>
                      ...
                      {block.isValid && <CheckCircle className="w-5 h-5 text-green-400" />}
                      {block.isMining && <Cpu className="w-5 h-5 text-orange-400 animate-spin" />}
                    </div>
                        ...
                        <div className="mt-2 space-y-1">
                          {block.transactions.map((tx) => (
                            <div key={tx.id} className="flex items-center">
                              <div className={`w-2 h-2 rounded-full mr-2 ${getTransactionColor(tx.type).replace('bg-', 'bg-')}`}></div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            <div className="text-center">
              ...
              <p className="text-slate-300 text-sm">
                Users create truth capsules containing verified information that gets permanently stored on the blockchain
              </p>
            </div>
            ...
            <div className="text-center">
              ...
              <p className="text-slate-300 text-sm">
                Community members verify capsules through consensus, earning GTT tokens for accurate verification
              </p>
            </div>
            ...
            <div className="text-center">
              ...
              <p className="text-slate-300 text-sm">
                Automated reward distribution based on verification accuracy and contribution to truth discovery
              </p>
            </div>
            ...
            <div className="text-center">
              ...
              <p className="text-slate-300 text-sm">
                Once verified and mined into blocks, truth capsules become permanent, tamper-proof records
              </p>
            </div>
          </div>
        </CardContent>
