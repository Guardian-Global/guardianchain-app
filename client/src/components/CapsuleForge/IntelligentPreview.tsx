...
import { Eye, Sparkles, Tag, TrendingUp, Shield, Globe, Users, DollarSign } from "lucide-react";
...
export default function IntelligentPreview({ formData, onOptimize }: IntelligentPreviewProps) {
  ...
  const analyzeContent = async () => {
    ...
    try {
      ...
      const mockAnalysis: ContentAnalysis = {
        ...
        suggestedTags: generateSuggestedTags(formData.content, formData.category),
  ...
  const generateSuggestedTags = (content: string, category: string): string[] => {
    const contentWords = content.toLowerCase().split(/\s+/);
    const commonWords = ['truth', 'evidence', 'verified', 'authentic', 'blockchain', 'decentralized'];
    const suggested = [];
  ...
  const getContentQuality = (content: string): ContentAnalysis["contentQuality"] => {
  ...
  const getAccessLevelIcon = (level: string) => {
    switch (level) {
      ...
      case "private": return <Shield className="h-4 w-4 text-red-400" />;
      case "restricted": return <Users className="h-4 w-4 text-yellow-400" />;
      case "premium": return <DollarSign className="h-4 w-4 text-purple-400" />;
      default: return <Globe className="h-4 w-4 text-green-400" />;
    }
  };
  ...
  const applyOptimizations = () => {
    ...
    const optimizations = {
      tags: [...formData.tags, ...analysis.suggestedTags.filter(tag => !formData.tags.includes(tag))],
      category: formData.category || "general",
      accessLevel: analysis.viralPotential > 70 ? "public" : formData.accessLevel,
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="bg-slate-900/30 rounded-lg p-3">
                ...
                <div className={`font-bold ${
                  analysis.contentQuality === "Excellent" ? "text-green-400" :
                  analysis.contentQuality === "High" ? "text-blue-400" :
                  analysis.contentQuality === "Medium" ? "text-yellow-400" : "text-red-400"
                }`}>
              ...
              <div className="bg-slate-900/30 rounded-lg p-3">
                ...
                <div className="text-green-400 font-bold">{analysis.truthScore}%</div>
              </div>
              
              <div className="bg-slate-900/30 rounded-lg p-3">
                <div className="text-sm text-slate-400">Viral Potential</div>
                <div className="text-purple-400 font-bold">{analysis.viralPotential}%</div>
              </div>
              
              <div className="bg-slate-900/30 rounded-lg p-3">
                <div className="text-sm text-slate-400">Engagement Est.</div>
                <div className="text-blue-400 font-bold">{analysis.estimatedEngagement}%</div>
              </div>
            </div>
              ...
              <div className="bg-blue-900/20 border border-blue-500/30 rounded-lg p-4">
                ...
                <div className="flex flex-wrap gap-1">
                  {analysis.suggestedTags.map(tag => (
                    <Badge key={tag} variant="secondary" className="bg-blue-900/50 text-blue-300 text-xs">
                      #{tag}
                    </Badge>
                  ))}
                </div>
              ...
              <div className="bg-green-900/20 border border-green-500/30 rounded-lg p-4">
                ...
                <div className="flex flex-wrap gap-1">
                  {analysis.keyPhrases.map(phrase => (
                    <Badge key={phrase} variant="secondary" className="bg-green-900/50 text-green-300 text-xs">
                      {phrase}
                    </Badge>
                  ))}
                </div>
