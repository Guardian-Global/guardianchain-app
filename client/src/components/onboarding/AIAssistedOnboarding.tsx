...
}> = ({ step, userInput, onApplySuggestion }) => {
...
// Personal Info Step
const PersonalInfoStep: React.FC<{ form: any; onNext: () => void }> = ({
  form,
  onNext,
}) => {
  return (
...
// Contact Info Step
const ContactInfoStep: React.FC<{ form: any; onNext: () => void }> = ({
  form,
}) => {
  return (
    <div className="space-y-6">
            ...
            <FormItem>
              ...
              <FormControl>
                <Input
                  {...field}
                  className="bg-slate-800 border-slate-600 text-white"
                  placeholder="GUARDIANCHAIN Corp"
                />
              </FormControl>
              <FormMessage />
            </FormItem>
    ...
    <div className="space-y-6">
      <div className="text-center mb-6">
        ...
        <p className="text-sm text-slate-400">
          Link your social accounts to build trust and credibility
        </p>
      </div>
...
const MediaStep: React.FC<{ form: any }> = ({ form }) => {
  ...
  const handleFileUpload = async (file: File, type: "picture" | "video") => {
    const setLoading =
      type === "picture" ? setUploadingPicture : setUploadingVideo;
    setLoading(true);
    ...
    try {
      ...
      if (result.success) {
        const fieldName =
          type === "picture" ? "profilePicture" : "profileVideo";
  ...
  return (
    <div className="space-y-6">
      <div className="text-center mb-6">
        ...
        <p className="text-sm text-slate-400">
          Add a profile picture and introduction video
        </p>
      </div>
...
export const AIAssistedOnboarding: React.FC<AIAssistedOnboardingProps> = ({
  onComplete,
}) => {
  ...
  const handleNext = async () => {
    ...
    if (currentStep < steps.length - 1) {
      ...
      toast({
        ...
        description:
          "Your profile is now complete. Enjoy exploring the platform!",
      });
      onComplete();
    }
  };
