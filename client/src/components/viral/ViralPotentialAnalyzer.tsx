...
const ViralPotentialAnalyzer: React.FC = () => {
  ...
  const analyzeViralPotential = async () => {
    ...
    const hasCallToAction = /share|comment|like|follow|subscribe|check out|click|visit/i.test(content);
    const hasTrendingWords = /ai|crypto|viral|trending|breaking|exclusive|secret|hack/i.test(content);

    const baseScore = Math.min(90, Math.max(20, 50 + (contentLength > 100 ? 10 : 0) + (hasEmojis ? 5 : 0) + (hasQuestions ? 5 : 0) + (hasCallToAction ? 10 : 0) + (hasTrendingWords ? 15 : 0)));

    const analysis: ViralMetrics = {
      content,
      emotionalResonance: Math.min(95, baseScore + Math.floor(Math.random() * 20) - 10),
      shareability: Math.min(95, baseScore + Math.floor(Math.random() * 20) - 10),
      trendAlignment: Math.min(95, baseScore + Math.floor(Math.random() * 20) - 10),
      audienceMatch: Math.min(95, baseScore + Math.floor(Math.random() * 20) - 10),
    ...
    analysis.overallViralScore = Math.round(
      (analysis.emotionalResonance + analysis.shareability + analysis.trendAlignment + analysis.audienceMatch + analysis.timingScore) / 5
    );
    ...
    const possibleHashtags = [
      '#viral', '#trending', '#content', '#social', '#share', '#engage',
      '#innovation', '#insights', '#growth', '#community', '#discussion',
      '#knowledge', '#tips', '#advice', '#inspiration', '#motivation'
    ];
    analysis.suggestedHashtags = possibleHashtags.slice(0, 5 + Math.floor(Math.random() * 3));
        ...
        <CardHeader>
          <CardTitle className="text-white flex items-center">
            ...
            <div>
              ...
              <div className="text-lg text-pink-400">AI-Powered Virality Prediction</div>
            </div>
          </CardTitle>
        </CardHeader>
        <CardContent>
          <div className="space-y-6">
            <div>
              <label className="text-white font-semibold mb-2 block">Content to Analyze</label>
              <Textarea
                value={content}
              ...
              <div className="text-sm text-slate-400 mt-1">
                {content.length} characters • Recommended: 100-280 for maximum engagement
              </div>
            </div>
          ...
          <Card className={`${getScoreBg(metrics.overallViralScore)}`}>
            <CardHeader>
              <CardTitle className="text-white flex items-center justify-between">
                ...
                <Badge className={`text-2xl px-4 py-2 ${getScoreColor(metrics.overallViralScore)}`}>
            ...
            <CardContent>
              <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
                <div className="text-center">
                  ...
                  <div className={`text-xl font-bold ${getScoreColor(metrics.emotionalResonance)}`}>
                    {metrics.emotionalResonance}%
                  </div>
                  <div className="text-sm text-slate-400">Emotional Impact</div>
                  <Progress value={metrics.emotionalResonance} className="mt-2 h-2" />
                </div>

                <div className="text-center">
                  <Share2 className="w-8 h-8 text-blue-400 mx-auto mb-2" />
                  <div className={`text-xl font-bold ${getScoreColor(metrics.shareability)}`}>
                ...
                <div className="text-center">
                  ...
                  <div className={`text-xl font-bold ${getScoreColor(metrics.trendAlignment)}`}>
                    {metrics.trendAlignment}%
                  </div>
                  <div className="text-sm text-slate-400">Trend Alignment</div>
                  <Progress value={metrics.trendAlignment} className="mt-2 h-2" />
                </div>

                <div className="text-center">
                  <Users className="w-8 h-8 text-green-400 mx-auto mb-2" />
                  <div className={`text-xl font-bold ${getScoreColor(metrics.audienceMatch)}`}>
                    {metrics.audienceMatch}%
                  </div>
                  <div className="text-sm text-slate-400">Audience Match</div>
                  <Progress value={metrics.audienceMatch} className="mt-2 h-2" />
                </div>

                <div className="text-center">
                  <Clock className="w-8 h-8 text-purple-400 mx-auto mb-2" />
                  <div className={`text-xl font-bold ${getScoreColor(metrics.timingScore)}`}>
                    {metrics.timingScore}%
                  </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
              ...
              <CardContent>
                <div className="space-y-4">
                  {metrics.bestPlatforms.map((platform, index) => (
                    <div key={platform} className="flex items-center justify-between p-3 bg-slate-700/50 rounded-lg">
                      <div className="flex items-center">
                  ...
                  <div className="mt-6">
                    <h4 className="text-white font-semibold mb-3">Optimal Timing</h4>
                    <div className="flex items-center p-3 bg-blue-900/20 rounded-lg border border-blue-700">
                      <Calendar className="w-5 h-5 text-blue-400 mr-2" />
                      <span className="text-blue-400 font-semibold">{metrics.optimalTiming}</span>
                    </div>
                  </div>
                </div>
              </CardContent>
            ...
            <Card className="bg-slate-800/50 border-slate-700">
              ...
              <CardContent>
                <div className="space-y-6">
                  <div>
                    <h4 className="text-white font-semibold mb-3">Suggested Hashtags</h4>
                    <div className="flex flex-wrap gap-2">
                      {metrics.suggestedHashtags.map((hashtag) => (
                        <Badge key={hashtag} className="bg-purple-600 text-white">
                          {hashtag}
                        </Badge>
                      ))}
                    </div>
                  ...
                  <div>
                    <h4 className="text-white font-semibold mb-3">Improvement Tips</h4>
          ...
          <Card className="bg-slate-800/50 border-slate-700">
            ...
            <CardContent>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                  <h3 className="text-white font-semibold mb-3">Content Strengths</h3>
                  <div className="space-y-2">
                    ...
                    {metrics.shareability >= 70 && (
                      <div className="flex items-center text-green-400">
                        <CheckCircle className="w-4 h-4 mr-2" />
                        <span className="text-sm">Strong shareability factors</span>
                      </div>
                    )}
                    {metrics.trendAlignment >= 70 && (
                      <div className="flex items-center text-green-400">
                        <CheckCircle className="w-4 h-4 mr-2" />
                        <span className="text-sm">Well-aligned with trends</span>
                      </div>
                    )}
                  ...
                  <div className="space-y-2">
                    {metrics.emotionalResonance < 70 && (
                      <div className="flex items-center text-yellow-400">
                        ...
                        <span className="text-sm">Boost emotional engagement</span>
                      </div>
                    )}
                    {metrics.shareability < 70 && (
                      <div className="flex items-center text-yellow-400">
                        <AlertTriangle className="w-4 h-4 mr-2" />
                        <span className="text-sm">Increase shareability elements</span>
                      </div>
                    )}
                    {metrics.trendAlignment < 70 && (
                      <div className="flex items-center text-yellow-400">
                        <AlertTriangle className="w-4 h-4 mr-2" />
                        <span className="text-sm">Better trend alignment needed</span>
                      </div>
                    )}
                    {metrics.audienceMatch < 70 && (
                      <div className="flex items-center text-yellow-400">
                        <AlertTriangle className="w-4 h-4 mr-2" />
                        <span className="text-sm">Refine audience targeting</span>
                      </div>
                    )}
                  </div>
                </div>
