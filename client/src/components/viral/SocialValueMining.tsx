  ...
  const mineValue = async () => {
    ...
    const platformData: MinedData[] = selectedPlatforms.map(platformId => {
      ...
      return {
        ...
        potentialEarnings: Math.floor((baseFollowers * 0.1) + (Math.random() * 5000)),
        dataPoints: {
    ...
    const totalValue = platformData.reduce((sum, p) => sum + p.estimatedValue, 0);
    const avgProtection = platformData.reduce((sum, p) => sum + p.protectionLevel, 0) / platformData.length;
        ...
        <CardHeader>
          <CardTitle className="text-white flex items-center">
            ...
            <div>
              ...
              <div className="text-lg text-orange-400">Discover Hidden Value in Your Social Presence</div>
            </div>
          </CardTitle>
        </CardHeader>
        <CardContent>
          <div className="space-y-6">
            <div>
              <label className="text-white font-semibold mb-2 block">Social Media Handle</label>
              <Input
                value={socialHandle}
            ...
            <div>
              <label className="text-white font-semibold mb-3 block">Select Platforms to Mine</label>
              <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
                {platforms.map((platform) => (
                  <Button
                    ...
                    className={`h-16 ${selectedPlatforms.includes(platform.id) ? platform.color : ''}`}
                  >
            ...
            <Button
              onClick={mineValue}
              disabled={mining || !socialHandle.trim() || selectedPlatforms.length === 0}
            ...
            <CardHeader>
              <CardTitle className="text-white flex items-center justify-between">
                ...
                <div className="text-right">
                  <div className="text-3xl font-bold text-green-400">${results.totalValue.toLocaleString()}</div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div className="text-center">
                  <div className="bg-blue-900/30 rounded-lg p-4 mb-3">
                    ...
                    <div className="text-2xl font-bold text-blue-400">{results.protectionScore}%</div>
                    <div className="text-sm text-slate-400">Protection Level</div>
                  </div>
                  <p className="text-slate-300 text-sm">Current content protection and ownership verification</p>
                </div>

                <div className="text-center">
                  <div className="bg-purple-900/30 rounded-lg p-4 mb-3">
                    <TrendingUp className="w-10 h-10 text-purple-400 mx-auto mb-2" />
                    <div className="text-2xl font-bold text-purple-400">{results.platforms.length}</div>
                    <div className="text-sm text-slate-400">Platforms Analyzed</div>
                  </div>
                  <p className="text-slate-300 text-sm">Social media platforms contributing to your value</p>
                </div>

                <div className="text-center">
                  <div className="bg-yellow-900/30 rounded-lg p-4 mb-3">
                    <Zap className="w-10 h-10 text-yellow-400 mx-auto mb-2" />
                    <div className="text-2xl font-bold text-yellow-400">{results.monetizationOpportunities.length}</div>
                    <div className="text-sm text-slate-400">Opportunities</div>
                  </div>
                  <p className="text-slate-300 text-sm">Monetization opportunities identified</p>
                </div>
              </div>
                  ...
                  <div className="space-y-4">
                    <div className="grid grid-cols-2 gap-4">
                      <div className="text-center p-3 bg-slate-700/50 rounded-lg">
                        ...
                        <div className="text-lg font-bold text-white">{platform.dataPoints.followers.toLocaleString()}</div>
                        <div className="text-xs text-slate-400">Followers</div>
                      </div>
                      <div className="text-center p-3 bg-slate-700/50 rounded-lg">
                        <Heart className="w-6 h-6 text-red-400 mx-auto mb-1" />
                        <div className="text-lg font-bold text-white">{platform.dataPoints.engagement}%</div>
                    ...
                    <div className="grid grid-cols-3 gap-2 text-center">
                      <div>
                        <div className="text-sm font-bold text-white">{platform.dataPoints.avgViews.toLocaleString()}</div>
                        <div className="text-xs text-slate-400">Avg Views</div>
                      </div>
                      <div>
                        <div className="text-sm font-bold text-white">{platform.dataPoints.avgLikes.toLocaleString()}</div>
                        <div className="text-xs text-slate-400">Avg Likes</div>
                      </div>
                      <div>
                        <div className="text-sm font-bold text-white">{platform.dataPoints.avgShares.toLocaleString()}</div>
                    ...
                    <div>
                      <div className="flex justify-between mb-2">
                        ...
                        <span className="text-white text-sm">{platform.protectionLevel}%</span>
                      </div>
                      <Progress value={platform.protectionLevel} className="h-2" />
                    </div>

                    <div className="bg-blue-900/20 rounded-lg p-3">
                      <div className="text-blue-400 font-semibold text-sm mb-2">Potential Monthly Earnings</div>
                      <div className="text-2xl font-bold text-green-400">${platform.potentialEarnings.toLocaleString()}</div>
                    </div>
                  </div>
                </CardContent>
              </Card>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
            <Card className="bg-slate-800/50 border-slate-700">
              ...
              <CardContent>
                <div className="space-y-4">
                  ...
                  <div>
                    <h4 className="text-white font-semibold mb-3">Monetization Opportunities</h4>
                    <div className="space-y-2">
                      {results.monetizationOpportunities.slice(0, 4).map((opp, index) => (
                        <div key={index} className="flex items-start">
            ...
            <Card className="bg-slate-800/50 border-slate-700">
              ...
              <CardContent>
                <div className="space-y-4">
                  <div>
                    <h4 className="text-white font-semibold mb-3">Risk Factors</h4>
                    <div className="space-y-2">
                  ...
                  <div className="bg-purple-900/20 rounded-lg p-4 border border-purple-700">
                    <div className="flex items-center mb-3">
                      ...
                      <span className="text-purple-400 font-semibold">GUARDIANCHAIN Protection</span>
                    </div>
                    <p className="text-slate-300 text-sm mb-3">
                      Protect your content and establish ownership before sharing on social media.
                    </p>
