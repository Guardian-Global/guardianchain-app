...
export default function PlayfulBlockchainAnimation() {
  ...
  const generateInitialTransactions = () => {
    const transactionTypes = [
      {
        type: "GTT_MINT" as const,
        data: "Mint 1000 GTT for truth verification",
      },
      ...
      setTimeout(() => {
        ...
        setStats((prevStats) => ({
          ...
          transactionsProcessed:
            prevStats.transactionsProcessed + pendingTxs.length,
          totalGTTMinted:
            prevStats.totalGTTMinted +
            pendingTxs.reduce((sum, tx) => sum + tx.amount, 0),
        }));
  ...
  const handleBlockMined = (blockData: any) => {
    setStats((prev) => ({
      ...
      transactionsProcessed:
        prev.transactionsProcessed +
        poolTransactions.filter((tx) => tx.status === "pending").length,
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-lg text-slate-300 max-w-2xl mx-auto">
            Interactive visualization of blockchain concepts through the
        ...
        <div className="flex justify-center gap-4 mb-8">
          ...
          <Button
            onClick={() => generateNewTransactions(3)}
            variant="outline"
            className="border-slate-600 text-white hover:bg-slate-800"
          >
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
          ...
          <Card className="bg-slate-800 border-slate-700 p-4">
            <div className="text-2xl font-bold text-yellow-400">
              {stats.totalGTTMinted.toLocaleString()}
            </div>
            <div className="text-sm text-slate-400">GTT Minted</div>
          </Card>
          <Card className="bg-slate-800 border-slate-700 p-4">
            <div className="text-2xl font-bold text-purple-400">
              {totalHashrate.toFixed(0)} TH/s
            </div>
                ...
                <div className="flex gap-4 overflow-x-auto pb-4">
                  {blocks.map((block, index) => (
                    <div key={block.id} className="flex-shrink-0">
                      <div
                        className={`w-32 h-24 rounded-lg border-2 p-3 ${
                          block.isBeingMined
              ...
              {interactiveBlocks.length === 0 ? (
                <Card className="bg-slate-800 border-slate-700 p-8 text-center">
                  ...
                  <p className="text-slate-400 mb-4">
                    Add your first block to begin understanding how blockchain
                    mining works
                  </p>
                  <Button
                    onClick={addInteractiveBlock}
                ...
                <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
                  {interactiveBlocks.map((block, index) => (
                    <InteractiveBlock
                      ...
                      isActive={
                        block.isActive || index === interactiveBlocks.length - 1
                      }
                    />
                  ))}
                </div>
          ...
          <Card className="bg-slate-800 border-slate-700 p-6">
            ...
            <div className="space-y-3 text-sm text-slate-300">
              <div className="flex items-start gap-3">
                ...
                <div>
                  <strong>Transactions:</strong> Users create truth capsules,
                  mint GTT tokens, and interact with the GUARDIANCHAIN protocol
                </div>
              </div>
              <div className="flex items-start gap-3">
                <div className="w-2 h-2 bg-yellow-400 rounded-full mt-2"></div>
                <div>
                  <strong>Mining:</strong> Guardian nodes validate transactions
                  and compete to create new blocks
                </div>
              </div>
              <div className="flex items-start gap-3">
                <div className="w-2 h-2 bg-blue-400 rounded-full mt-2"></div>
                <div>
                  <strong>Consensus:</strong> The network agrees on the longest
              ...
              <div className="flex items-start gap-3">
                ...
                <div>
                  <strong>Rewards:</strong> Miners earn GTT tokens for securing
                  the network
                </div>
              </div>
            </div>
          ...
          <Card className="bg-slate-800 border-slate-700 p-6">
            ...
            <div className="space-y-3 text-sm">
              ...
              <div className="flex justify-between">
                ...
                <span className="text-white font-mono">
                  {miners.filter((m) => m.isActive).length}/{miners.length}
                </span>
              </div>
              <div className="flex justify-between">
              ...
              <div className="flex justify-between">
                ...
                <span className="text-green-400 font-mono">
                  ${(stats.totalGTTMinted * 0.025).toFixed(2)}
                </span>
              </div>
            </div>
          </Card>
