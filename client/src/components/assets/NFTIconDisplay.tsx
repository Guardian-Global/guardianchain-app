...
export const NFTIconDisplay: React.FC<NFTIconDisplayProps> = ({
  ...
  if (loading) {
    return (
      <div
        className={`${sizeClasses[size]} bg-slate-700 animate-pulse rounded-full ${className}`}
      />
    );
  }
    ...
    <div className={`${sizeClasses[size]} relative ${className}`}>
      ...
      <div
        className={`absolute inset-0 bg-gradient-to-br ${
          rarityGradients[rarity]
        } rounded-full opacity-20 blur-sm ${animated ? "animate-pulse" : ""}`}
      />
      ...
      <img
        ...
        alt={`${type} ${rarity} NFT`}
        className={`w-full h-full object-cover rounded-full border-2 border-gradient-to-br ${
          rarityGradients[rarity]
      ...
      <div className="absolute -top-1 -right-1">
        <div
          className={`w-4 h-4 bg-gradient-to-br ${rarityGradients[rarity]} rounded-full border border-white flex items-center justify-center`}
        >
