...
import { TrendingUp, TrendingDown, Eye, AlertTriangle, Zap, Target } from 'lucide-react';
...
export function WhaleTracker() {
  ...
  const [activeTab, setActiveTab] = useState<'transactions' | 'wallets' | 'alerts'>('transactions');

  useEffect(() => {
    // Simulate real-time whale activity
    const generateWhaleData = () => {
      const exchanges = ['Binance', 'Coinbase', 'OKX', 'Kraken', 'Uniswap V3'];
      const walletPrefixes = ['0x742d...', '0x959C...', '0x8c7C...', '0xa1b2...', '0xdef0...'];
  ...
  const getTierColor = (tier: string) => {
    switch (tier) {
      ...
      case 'smart_money': return 'bg-blue-500';
              ...
              {whaleTransactions.map((tx) => (
                ...
                >
                  <div className="flex items-center gap-3">
                    <div className={`p-2 rounded ${tx.type === 'buy' ? 'bg-green-500/20' : 'bg-red-500/20'}`}>
                      {tx.type === 'buy' ? (
              ...
              {smartWallets.map((wallet) => (
                ...
                >
                  <div className="flex items-start justify-between mb-3">
                    <div>
                      <div className="flex items-center gap-2 mb-1">
                        <span className="text-white font-medium">{wallet.nickname}</span>
                        <div className={`w-2 h-2 rounded-full ${getTierColor(wallet.tier)}`} />
                        <Badge variant="outline" className="text-xs">
                  ...
                  <div className="grid grid-cols-4 gap-4 text-center">
                    ...
                    <div>
                      <div className="text-white font-medium">{wallet.avgHoldTime}</div>
                    ...
                    <div>
                      <div className="text-blue-400 font-medium">
                        {Math.floor((Date.now() - wallet.lastActivity.getTime()) / 60000)}m
                      </div>
          ...
          {activeTab === 'alerts' && (
            <div className="space-y-4">
              ...
              <div className="space-y-3">
                <div className="flex items-center justify-between p-3 bg-slate-800 rounded-lg">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <div className="text-white font-medium">Large Buy Orders</div>
                      <div className="text-sm text-slate-400">Alert on purchases &gt; $10K</div>
                    </div>
                  </div>
                  <Button
                    size="sm"
                ...
                <div className="flex items-center justify-between p-3 bg-slate-800 rounded-lg">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <div className="text-white font-medium">Smart Money Moves</div>
                      <div className="text-sm text-slate-400">Track 90%+ win rate wallets</div>
                    </div>
                  </div>
                ...
                <div className="flex items-center justify-between p-3 bg-slate-800 rounded-lg">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <div className="text-white font-medium">Institutional Activity</div>
                      <div className="text-sm text-slate-400">Monitor whale accumulation patterns</div>
                    </div>
                  </div>
                  <Button size="sm" variant="outline">
                    Configure
              ...
              <div className="mt-6 p-4 bg-purple-500/10 rounded-lg border border-purple-500/20">
                <div className="text-purple-400 font-medium mb-2">API Access Available</div>
                <div className="text-sm text-slate-300">
                  Get real-time whale alerts via webhook or WebSocket for trading bot integration.
                  Enterprise plans include custom alert thresholds and advanced analytics.
                </div>
