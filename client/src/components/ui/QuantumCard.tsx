import React from 'react';
import { cn } from '@/lib/utils';

interface QuantumCardProps extends React.HTMLAttributes<HTMLDivElement> {
  variant?: 'default' | 'holographic' | 'neural' | 'quantum';
  children: React.ReactNode;
}

export function QuantumCard({ 
  className, 
  variant = 'default', 
  children, 
  ...props 
}: QuantumCardProps) {
  const variants = {
    default: 'bg-card/80 border-border/50 backdrop-blur-xl',
    holographic: 'holographic-glass border-white/20',
    neural: 'bg-gradient-to-br from-purple-900/20 via-blue-900/20 to-cyan-900/20 border-cyan-500/30 neural-pulse',
    quantum: 'quantum-field border-yellow-500/30 backdrop-blur-xl'
  };

  return (
    <div
      className={cn(
        'rounded-2xl border p-6 transition-all duration-300 hover:scale-[1.02] hover:shadow-2xl',
        variants[variant],
        className
      )}
      {...props}
    >
      {children}
    </div>
  );
}

export default QuantumCard;