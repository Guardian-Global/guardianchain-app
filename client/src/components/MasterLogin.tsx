  ...
  const roles = [
    {
      ...
      permissions: ['GTT Minting', 'Capsule Sealing', 'Treasury Management', 'System Administration']
    },
    {
      id: 'founder',
      title: 'Founder',
      ...
      permissions: ['Financial Overview', 'Strategic Planning', 'Team Management', 'Compliance Monitoring']
    },
    {
      id: 'architect',
      title: 'Architect',
      ...
      permissions: ['System Architecture', 'Smart Contracts', 'API Management', 'Security Protocols']
    }
  ];
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-purple-900 to-slate-900 flex items-center justify-center p-4">
      <Card className="w-full max-w-4xl bg-slate-800/80 border-slate-700 backdrop-blur-lg">
        <CardHeader className="text-center pb-8">
          ...
          <CardTitle className="text-3xl font-bold text-white mb-2">Master Access Portal</CardTitle>
          <CardDescription className="text-slate-300 text-lg">
            Select your access level and authenticate to enter the GUARDIANCHAIN protocol
          </CardDescription>
        </CardHeader>
        ...
        <CardContent className="space-y-8">
          {/* Role Selection */}
          <div>
            <Label className="text-white text-lg font-semibold mb-4 block">Access Level</Label>
                ...
                return (
                  ...
                  >
                    <CardContent className="p-6 text-center">
                      ...
                      <p className="text-slate-300 text-sm mb-4">{role.description}</p>
                      <div className="space-y-1">
                        {role.permissions.slice(0, 2).map((perm, i) => (
                          <div key={i} className="text-xs text-green-400">✓ {perm}</div>
                        ))}
                        {role.permissions.length > 2 && (
                          <div className="text-xs text-slate-400">+{role.permissions.length - 2} more</div>
                        )}
                      </div>
                    </CardContent>
                  </Card>
          ...
          {selectedRole && (
            <div className="bg-slate-700/30 rounded-lg p-6 border border-slate-600">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                  <Label className="text-white mb-2 block">Master Password</Label>
                  <Input
                    type="password"
                    placeholder="••••••••••••"
                ...
                <div className="md:col-span-2">
                  <Label className="text-white mb-2 block">Master Access Key</Label>
                  <Input
                    type="password"
                  ...
                  <p className="text-xs text-slate-400 mt-1">
                    This is your protocol-level access key for {selectedRole} privileges
                  </p>
                </div>
              </div>
            </div>
          ...
          {selectedRole && (
            <div className="flex justify-center">
              <Button 
                onClick={handleLogin}
                disabled={!credentials.email || !credentials.password || !credentials.masterKey || loading}
              ...
              >
                {loading ? (
                  ...
                  <div className="flex items-center space-x-2">
                    ...
                    <span>Access Protocol as {roles.find(r => r.id === selectedRole)?.title}</span>
                  </div>
                )}
              </Button>
            </div>
