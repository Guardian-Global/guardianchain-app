...
import { 
  FileText, Scale, Brain, Palette, Users, TrendingUp, 
  Shield, Bot, Link, Film, Newspaper, AlertTriangle, 
  UserCheck, Heart, Check, Star, Zap 
} from 'lucide-react';
...
const getTypeIcon = (type: CapsuleType) => {
  switch (type) {
    ...
    case 'CIVIC': return Users;
    case 'FINANCIAL': return TrendingUp;
    case 'VERITAS_CERTIFICATE': return Shield;
    case 'AI_GENERATED': return Bot;
    case 'CROSS_CHAIN_ASSET': return Link;
    case 'MULTIMEDIA_STORY': return Film;
    case 'CITIZEN_JOURNALISM': return Newspaper;
    case 'FRAUD_PROOF': return AlertTriangle;
    case 'WITNESS_TESTIMONY': return UserCheck;
    case 'SOULBOUND_MEMOIR': return Heart;
      ...
      <CardHeader>
        <CardTitle className="flex items-center gap-2">
          <div className="p-2 rounded-lg" style={{ backgroundColor: `${BRAND_COLORS.GUARDIAN}20` }}>
            <Shield className="h-5 w-5" style={{ color: BRAND_COLORS.GUARDIAN }} />
          </div>
          <div>
          ...
          <div className="bg-slate-700/30 rounded-lg p-4 border border-slate-600">
            <div className="flex items-center justify-between mb-3">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="text-white font-semibold">{selectedConfig.name}</h3>
                  <p className="text-slate-400 text-sm">{selectedConfig.description}</p>
                </div>
              </div>
              <div className="text-right">
                <div className={`font-bold text-lg ${getFeeCSSClass(selectedConfig.baseFee + selectedConfig.premiumFee)}`}>
                ...
                <CardContent className="p-4">
                  <div className="flex items-start justify-between mb-3">
                    <div className="flex items-center gap-2">
                      <div className={`p-2 rounded-lg ${isSelected ? 'bg-purple-600' : 'bg-slate-700'}`}>
                        <Icon className="w-4 h-4 text-white" />
                      </div>
                      <div>
                        <h4 className="text-white font-semibold text-sm">{config.name}</h4>
                        <div className={`font-semibold text-sm ${getFeeCSSClass(totalFee)}`}>
                          {totalFee} GTT
                        </div>
                      </div>
                    </div>
                  ...
                  <div className="flex flex-wrap gap-1 mb-3">
                    {config.features.slice(0, 2).map((feature, idx) => (
                      <Badge key={idx} variant="outline" className="border-slate-600 text-slate-400 text-xs px-1 py-0">
                        {feature}
                      </Badge>
                    ))}
                    {config.features.length > 2 && (
                      <Badge variant="outline" className="border-slate-600 text-slate-500 text-xs px-1 py-0">
                        +{config.features.length - 2}
                      </Badge>
                    )}
                  </div>
                    ...
                    <div className="mt-3 pt-3 border-t border-slate-700">
                      <div className="space-y-2">
                        <div className="text-xs">
                          ...
                          <span className="text-white ml-1">{config.baseFee} GTT</span>
                        </div>
                        <div className="text-xs">
                          <span className="text-slate-500">Premium Fee:</span>
                          <span className="text-white ml-1">{config.premiumFee} GTT</span>
                        </div>
          ...
          <div className="bg-slate-700/20 rounded-lg p-4">
            ...
            <div className="space-y-2 text-sm">
              ...
              <div className="flex justify-between">
                ...
                <span className="text-white">{selectedConfig.premiumFee} GTT</span>
              </div>
              <div className="border-t border-slate-600 pt-2 flex justify-between font-semibold">
                <span className="text-white">Total Cost</span>
                <span className={getFeeCSSClass(selectedConfig.baseFee + selectedConfig.premiumFee)}>
                  {selectedConfig.baseFee + selectedConfig.premiumFee} GTT
                </span>
              </div>
            </div>
