...
export function TokenLaunchpadPro() {
  const [activeTab, setActiveTab] = useState<
    "live" | "completed" | "upcoming" | "analytics"
  >("live");
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-slate-400">
            {formatCurrency(metrics.totalRaised)} raised across{" "}
            {metrics.totalProjectsLaunched} successful launches
          </p>
        </div>
          ...
          <CardContent className="p-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xl font-bold text-white">
                  {formatNumber(metrics.totalProjectsLaunched)}
                </p>
              </div>
            </div>
          </CardContent>
      ...
      {activeTab === "live" && (
        <div className="space-y-4">
          ...
          <div className="grid gap-6">
            {projects
              .filter((p) => p.status === "live" || p.status === "sold_out")
              .map((project) => (
                <Card
                  key={project.id}
                        ...
                        <div className="mb-4">
                          <div className="flex justify-between text-sm mb-2">
                            ...
                            <span className="text-white">
                              {(
                                (project.currentRaise / project.totalRaise) *
                                100
                              ).toFixed(1)}
                              %
                            </span>
                          ...
                          <Progress
                            value={
                              (project.currentRaise / project.totalRaise) * 100
                            }
                            className="h-2 mb-2"
                          />
        ...
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            ...
            <Badge className="bg-purple-600/20 text-purple-400">
              {projects.filter((p) => p.status === "upcoming").length} Projects
              in Pipeline
            </Badge>
          </div>
                  ...
                  {pricingTiers.map((tier, index) => (
                    <div key={tier.name} className="space-y-2">
                      ...
                      <Progress
                        value={
                          (tier.revenue /
                            Math.max(...pricingTiers.map((t) => t.revenue))) *
                          100
                        }
                        className="h-2"
                      />
                ...
                <div className="space-y-4">
                  <div className="flex justify-between">
                    ...
                    <span className="text-white font-medium">
                      {formatCurrency(
                        metrics.totalRaised / metrics.totalProjectsLaunched,
                      )}
                    </span>
                  </div>
                  <div className="flex justify-between">
                    ...
                    <span className="text-green-400 font-bold">
                      {formatCurrency(metrics.totalRaised * 0.025)}{" "}
                      {/* 2.5% platform fee */}
                    </span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-slate-400">
                      Tier Subscription Revenue
                    </span>
                    <span className="text-green-400 font-bold">
                      {formatCurrency(
                        pricingTiers.reduce(
              ...
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-purple-300 mb-4">
                    Industry-leading {metrics.successRate}% success rate with{" "}
                    {metrics.averageROI}% average ROI
                  </p>
                  <div className="flex items-center gap-4">
                    <ArrowUpRight className="h-5 w-5 text-green-400" />
                    <span className="text-sm text-green-300">
                      {formatCurrency(metrics.monthlyRevenue)} monthly recurring
                      revenue
                    </span>
                  </div>
                </div>
