...
export function EnterpriseDAOSuite() {
  const [activeTab, setActiveTab] = useState<
    "clients" | "governance" | "revenue" | "analytics"
  >("clients");
  ...
  const totalTreasuryValue = daoClients.reduce(
    (sum, client) => sum + client.treasuryValue,
    0,
  );
  const totalMonthlyRevenue = revenueStreams.reduce(
    (sum, stream) => sum + stream.monthlyRevenue,
    0,
  );
  const totalMembers = daoClients.reduce(
    (sum, client) => sum + client.memberCount,
    0,
  );
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold text-white mb-2">
            Enterprise DAO Suite
          </h2>
          <p className="text-slate-400">
            Managing {formatCurrency(totalTreasuryValue)} in DAO treasuries
            across {daoClients.length} enterprise clients
          </p>
        </div>
                      ...
                      <div className="grid grid-cols-4 gap-4 mb-4">
                        <div>
                          ...
                          <p className="text-lg font-bold text-white">
                            {formatCurrency(client.treasuryValue)}
                          </p>
                        </div>
                        <div>
              ...
              <div className="flex items-center justify-between">
                <div>
                  <h4 className="text-xl font-bold text-white mb-2">
                    Enterprise DAO Package
                  </h4>
                  <p className="text-sm text-purple-300 mb-4">
                    Complete enterprise DAO solution with custom governance,
                  ...
                  <div className="flex items-start justify-between">
                    <div className="flex-1">
                      ...
                      <div className="space-y-2">
                        ...
                        <Progress
                          value={
                            (proposal.forVotes / proposal.votingPower) * 100
                          }
                          className="h-2"
                        />
                        <div className="flex justify-between text-xs text-slate-500">
                          <span>
                            Total Voting Power:{" "}
                            {formatNumber(proposal.votingPower)}
                          </span>
                          <span>
                            Participation:{" "}
                            {(
                ...
                <div className="space-y-4">
                  {revenueStreams.map((stream, index) => (
                    <div key={index} className="space-y-2">
                      ...
                      <Progress
                        value={
                          (stream.monthlyRevenue / totalMonthlyRevenue) * 100
                        }
                        className="h-2"
                      />
                      <div className="flex justify-between text-xs text-slate-500">
                        ...
                        <span>
                          {(
                            (stream.monthlyRevenue / totalMonthlyRevenue) *
                            100
                          ).toFixed(1)}
                          %
                        </span>
                ...
                <div className="space-y-4">
                  <div className="flex justify-between">
                    ...
                    <span className="text-white font-medium">
                      {formatCurrency(
                        (totalMonthlyRevenue * 12) / daoClients.length,
                      )}
                    </span>
                  </div>
                  <div className="flex justify-between">
                  ...
                  <div className="flex justify-between">
                    <span className="text-slate-400">
                      Average Members per DAO
                    </span>
                    <span className="text-white font-medium">
                      {formatNumber(totalMembers / daoClients.length)}
                    </span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-slate-400">
          ...
          <Card className="bg-gradient-to-br from-purple-900/50 to-green-900/50 border-purple-500/30">
            <CardContent className="p-6">
              <div className="flex items-center justify-between">
                <div>
                  <h4 className="text-xl font-bold text-white mb-2">
                    Enterprise DAO Market Leadership
                  </h4>
                  <p className="text-purple-300 mb-4">
                    Managing {formatCurrency(totalTreasuryValue)} across{" "}
                    {daoClients.length} enterprise DAOs with 100% client
                    retention
                  </p>
                  <div className="flex items-center gap-4">
                    <ArrowUpRight className="h-5 w-5 text-green-400" />
                    <span className="text-sm text-green-300">
                      {formatCurrency(totalMonthlyRevenue * 12)} annual
                      recurring revenue
                    </span>
                  </div>
                </div>
