...
export function EmotionProvider({ children }: EmotionProviderProps) {
  ...
  return (
    <EmotionContext.Provider
      value={{ currentEmotion, setEmotion, theme, analyzeContent }}
    >
      {children}
    </EmotionContext.Provider>
  );
...
export function EmotionAnimatedBackground({
  children,
}: EmotionAnimatedBackgroundProps) {
    ...
    switch (currentEmotion) {
      ...
      default:
        return {
          animate: {
            background:
              "radial-gradient(circle at 50% 50%, rgba(71, 85, 105, 0.05) 0%, transparent 70%)",
          },
        };
    }
  };
