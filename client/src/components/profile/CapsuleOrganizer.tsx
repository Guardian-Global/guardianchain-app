      ...
      {viewMode === 'grid' ? (
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredCapsules.map((capsule) => {
            ...
            return (
              <Card key={capsule.id} className="bg-slate-800 border-slate-700 hover:bg-slate-750 transition-colors">
                <CardHeader className="pb-3">
                  <div className="flex items-start justify-between">
                    ...
                    <Badge variant="outline" className={`${capsule.privacy === 'private' ? 'border-red-400 text-red-400' : 'border-green-400 text-green-400'}`}>
                      {capsule.privacy}
                    </Badge>
                  </div>
                  ...
                  <div className="space-y-3">
                    <div className="grid grid-cols-2 gap-4 text-sm">
                      <div>
                        ...
                        <span className="text-amber-400">{capsule.gttValue.toLocaleString()} GTT</span>
                      </div>
                      
                      <div>
                        <span className="text-slate-400">Yield: </span>
                        <span className="text-green-400">{capsule.yieldGenerated.toLocaleString()} GTT</span>
                      </div>
                    </div>
                    ...
                    <div className="grid grid-cols-2 gap-4 text-sm">
                      <div>
                        ...
                        <span className="text-white">{new Date(capsule.createdDate).toLocaleDateString()}</span>
                      </div>
                      
                      <div>
                        <span className="text-slate-400">Verified: </span>
                        <span className="text-blue-400">{capsule.verificationCount}x</span>
                      </div>
                    </div>
                ...
                return (
                  <div
                    ...
                    className={`p-4 flex items-center justify-between hover:bg-slate-750 transition-colors ${
                      index !== filteredCapsules.length - 1 ? 'border-b border-slate-700' : ''
                    }`}
                  >
                    <div className="flex items-center space-x-4">
                      <TypeIcon className="w-5 h-5 text-purple-400" />
                    ...
                    <div className="flex items-center space-x-4">
                      <div className="text-right">
                        <div className="text-amber-400 font-medium">{capsule.gttValue.toLocaleString()} GTT</div>
                        <div className="text-green-400 text-sm">+{capsule.yieldGenerated.toLocaleString()} yield</div>
                      </div>
      ...
      {filteredCapsules.length === 0 && (
        <Card className="bg-slate-800 border-slate-700">
          <CardContent className="p-12 text-center">
            ...
            <h3 className="text-lg font-medium text-white mb-2">No capsules found</h3>
