...
interface ValidationTest {
  id: string;
  category: 'authentication' | 'capsule' | 'yield' | 'export' | 'responsive' | 'error';
  name: string;
  description: string;
...
export default function SystemValidator() {
  ...
  const testCapsuleMint = async () => {
    try {
      const testCapsule = {
        ...
        content: 'This capsule contains test content with emotional weight for grief scoring validation.',
  ...
  const testYieldClaim = async () => {
    updateTest('yield-claim', { 
      status: 'warning', 
      result: 'Manual test required - attempt yield claim with claimable capsule'
    });
  };
  ...
  const testExplorerFilters = async () => {
    try {
      const response = await fetch('/api/capsules/explore?verificationStatus=verified');
      const data = await response.json();
  ...
  const testMobileCompatibility = async () => {
    ...
    updateTest('mobile-modules', { 
      ...
      result: isMobile ? 'Running on mobile device' : 'Desktop detected - mobile test needed'
    });
  };
  ...
  const getCategoryIcon = (category: string) => {
    switch (category) {
      ...
      case 'capsule': return <FileText className="w-4 h-4" />;
      case 'yield': return <DollarSign className="w-4 h-4" />;
              ...
              {categoryTests.map((test) => (
                <div 
                  ...
                  className={`p-3 rounded-lg border transition-colors ${
                    ...
                    test.status === 'warning' ? 'border-yellow-500/50 bg-yellow-500/10' :
                ...
                >
                  <div className="flex items-start justify-between">
                    <div className="flex-1">
                      ...
                      {test.result && (
                        <p className="text-green-300 text-sm">✓ {test.result}</p>
                      )}
