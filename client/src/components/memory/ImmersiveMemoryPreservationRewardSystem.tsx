...
export default function ImmersiveMemoryPreservationRewardSystem() {
  ...
  const progressToNext = nextTier
    ? ((currentXP - currentTier.requiredXP) /
        (nextTier.requiredXP - currentTier.requiredXP)) *
      100
    : 100;
  ...
  const AchievementCard = ({ achievement }: { achievement: Achievement }) => {
    ...
    return (
      ...
      >
        <Card
          className={`bg-slate-800/80 backdrop-blur-sm border-2 ${rarityBorders[achievement.rarity]} overflow-hidden`}
        >
          <CardHeader className="pb-3">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                <Icon
                  className={`${isCompleted ? "text-yellow-400" : "text-gray-400"}`}
                  size={20}
                />
                {isCompleted && (
  ...
  const DailyChallengeCard = ({ challenge }: { challenge: DailyChallenge }) => {
    ...
    return (
      <Card
        className={`bg-slate-800/80 backdrop-blur-sm border transition-all duration-300 ${
          challenge.completed
      ...
      <div className="max-w-7xl mx-auto">
        ...
        >
          ...
          <p className="text-gray-300 text-lg max-w-2xl mx-auto">
            Earn rewards, unlock achievements, and level up your memory
                  ...
                  {nextTier && (
                    <div className="mt-2">
                      <div className="text-xs text-gray-400 mb-1">
                        Next Level: {nextTier.requiredXP.toLocaleString()} XP
                      </div>
                  ...
                  {nextTier && (
                    <div className="mt-4 pt-4 border-t border-slate-600">
                      <h4 className="text-white font-semibold mb-2">
                        Next Tier: {nextTier.title}
                      </h4>
                      <div className="space-y-2">
                        {nextTier.benefits
                          .slice(currentTier.benefits.length)
                          .map((benefit, index) => (
                            <div
                              key={index}
            ...
            >
              ...
              >
                <div className="text-center">
                  ...
                  <Badge
                    className={`mb-4 bg-gradient-to-r ${rarityColors[selectedAchievement.rarity]} text-white`}
                  >
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="flex items-center justify-center gap-2">
                      ...
                      <span className="text-yellow-300 font-bold text-lg">
                        {selectedAchievement.reward} GTT Reward
                      </span>
                    </div>
