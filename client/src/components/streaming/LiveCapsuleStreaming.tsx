...
interface StreamData {
  ...
  category: 'institutional' | 'earth' | 'cultural' | 'sovereign' | 'technological';
  viewers: number;
  duration: number;
...
export function LiveCapsuleStreaming() {
  const [activeTab, setActiveTab] = useState<'live' | 'scheduled' | 'browse' | 'following'>('live');
  ...
  const [liveStreams] = useState<StreamData[]>([
    {
      id: '1',
      title: 'Goldman Sachs Institutional Legacy Vault Creation - Live Documentation',
      description: 'Exclusive behind-the-scenes look at creating a $75M institutional legacy vault for Fortune 500 executives.',
      streamer: {
        id: 'goldman_team',
      ...
      status: 'live',
      quality: '4K',
      tags: ['institutional', 'fortune500', 'legacy', 'enterprise', 'governance'],
    ...
    },
    {
      ...
      description: 'Live satellite feeds and on-ground conservation efforts for our $100M rainforest preservation project.',
      streamer: {
      ...
      status: 'live',
      quality: '1080p',
      tags: ['environment', 'conservation', 'rainforest', 'climate', 'biodiversity'],
    ...
    {
      id: '3',
      title: 'Mayan Archaeological Site Digital Preservation - 3D Scanning Live',
      description: 'Watch as we create high-resolution 3D models of ancient Mayan ruins for our cultural legacy archive.',
      streamer: {
        id: 'maya_heritage',
    ...
    },
    {
      ...
      description: 'Live from OpenAI research facilities: documenting breakthrough AI developments for our $8.5B tech legacy.',
      streamer: {
        id: 'openai_research',
      ...
      status: 'live',
      quality: '4K',
      tags: ['ai', 'research', 'technology', 'breakthrough', 'superintelligence'],
    ...
    },
    {
      ...
      description: 'Historic documentation of constitutional preservation process for the U.S. Constitutional Archive project.',
      streamer: {
        id: 'us_archives',
      ...
      status: 'live',
      quality: '1080p',
      tags: ['constitution', 'government', 'history', 'sovereignty', 'democracy'],
  ...
  const getCategoryColor = (category: string) => {
    switch (category) {
      case 'institutional': return 'bg-purple-600/20 text-purple-400';
      case 'earth': return 'bg-green-600/20 text-green-400';
      case 'cultural': return 'bg-blue-600/20 text-blue-400';
      case 'sovereign': return 'bg-yellow-600/20 text-yellow-400';
      case 'technological': return 'bg-red-600/20 text-red-400';
  ...
  const getTierColor = (tier: string) => {
    switch (tier) {
      ...
      case 'seeker': return 'text-blue-400';
  ...
  const filteredStreams = liveStreams.filter(stream => {
    ...
    if (filters.quality !== 'all' && stream.quality !== filters.quality) return false;
    if (filters.language !== 'all' && stream.language !== filters.language) return false;
    return true;
  }).sort((a, b) => {
    switch (filters.sortBy) {
      case 'viewers': return b.viewers - a.viewers;
      case 'duration': return b.duration - a.duration;
      case 'recent': return b.startTime.getTime() - a.startTime.getTime();
      case 'trending': return b.viewers - a.viewers; // Simplified trending
      default: return 0;
    }
  });
    ...
    <div className="min-h-screen bg-slate-900 text-white">
      {selectedStream ? (
        ...
        <div className={`${isFullscreen ? 'fixed inset-0 z-50' : ''} flex h-screen`}>
          {/* Video Player */}
          <div className={`${isFullscreen ? 'w-full' : 'flex-1'} bg-black flex flex-col`}>
            {/* Video Container */}
              ...
              <div className="p-4 bg-slate-800">
                <div className="flex items-start justify-between">
                  <div className="flex-1">
                    <h2 className="text-xl font-bold mb-2">{selectedStreamData?.title}</h2>
                    <p className="text-slate-400 mb-3">{selectedStreamData?.description}</p>
                    
                    <div className="flex items-center space-x-4">
                      <div className="flex items-center space-x-2">
                        <Avatar className="h-8 w-8">
                          <AvatarImage src={selectedStreamData?.streamer.avatar} />
                          <AvatarFallback>{selectedStreamData?.streamer.name.slice(0, 2)}</AvatarFallback>
                        </Avatar>
                        <div>
                          <div className="flex items-center space-x-1">
                            <span className="font-medium">{selectedStreamData?.streamer.name}</span>
                            {selectedStreamData?.streamer.verified && <Shield className="h-4 w-4 text-blue-400" />}
                          </div>
                          <span className={`text-sm ${getTierColor(selectedStreamData?.streamer.tier || '')}`}>
                      ...
                      <div className="flex items-center space-x-4 text-sm text-slate-400">
                        <div className="flex items-center space-x-1">
                          ...
                          <span>{formatViewerCount(selectedStreamData?.viewers || 0)} viewers</span>
                        </div>
                        <Badge className={getCategoryColor(selectedStreamData?.category || '')}>
              ...
              <div className="p-4 border-b border-slate-700">
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex items-center space-x-1 text-sm text-slate-400">
                    ...
                    <span>{formatViewerCount(selectedStreamData?.viewers || 0)}</span>
                  </div>
                </div>
              </div>
              ...
              <div className="flex-1 overflow-y-auto p-4 space-y-3">
                {chatMessages.map((msg) => (
                  <div key={msg.id} className="flex items-start space-x-2">
                    <div className="flex-1">
                      <div className="flex items-center space-x-2">
                        <span className={`text-sm font-medium ${getTierColor(msg.tier)}`}>
                          {msg.username}
                        </span>
        ...
        <div className="max-w-7xl mx-auto p-6">
          ...
          <div className="flex items-center justify-between mb-6">
            <div>
              <h1 className="text-3xl font-bold mb-2">Live Capsule Streaming</h1>
              <p className="text-slate-400">
                Watch live creation and documentation of legacy capsules from around the world
              </p>
            </div>
                ...
                <div className="grid grid-cols-4 gap-4">
                  ...
                  <div className="flex items-end">
                    <Button 
                      size="sm" 
                      variant="outline" 
                      onClick={() => setFilters({category: 'all', quality: 'all', language: 'all', sortBy: 'viewers'})}
                      className="w-full"
                    >
                      Clear Filters
                    </Button>
                ...
                <CardContent className="p-4">
                  ...
                  <div className="flex items-center space-x-2 mb-3">
                    <Avatar className="h-6 w-6">
                      ...
                      <AvatarFallback>{stream.streamer.name.slice(0, 2)}</AvatarFallback>
                    </Avatar>
                    <div className="flex items-center space-x-1">
                      <span className="text-sm font-medium">{stream.streamer.name}</span>
                      {stream.streamer.verified && <Shield className="h-3 w-3 text-blue-400" />}
                    </div>
                  </div>
