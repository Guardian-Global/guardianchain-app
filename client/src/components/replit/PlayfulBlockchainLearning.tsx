...
export default function PlayfulBlockchainLearning() {
  ...
  const startLearning = async (moduleId: string) => {
    ...
    for (let i = 0; i < Math.min(conceptsToMark, availableConcepts.length); i++) {
        ...
        <CardContent className="space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            <div className="bg-slate-700/50 rounded-lg p-4 text-center">
              <p className="text-2xl font-bold text-purple-400">{Math.round(overallProgress)}%</p>
              <p className="text-sm text-slate-400">Overall Progress</p>
            </div>
            <div className="bg-slate-700/50 rounded-lg p-4 text-center">
              <p className="text-2xl font-bold text-green-400">{totalGTTEarned}</p>
              <p className="text-sm text-slate-400">GTT Earned</p>
            </div>
            <div className="bg-slate-700/50 rounded-lg p-4 text-center">
              <p className="text-2xl font-bold text-blue-400">{conceptsUnderstood}</p>
              <p className="text-sm text-slate-400">Concepts Mastered</p>
            </div>
            <div className="bg-slate-700/50 rounded-lg p-4 text-center">
              <p className="text-2xl font-bold text-orange-400">{learningStreak}</p>
          ...
          {isLearning && currentModule && (
            <div className="space-y-2">
              <div className="flex justify-between text-sm">
                <span className="text-slate-300">Learning: {modules.find(m => m.id === currentModule)?.title}</span>
                <span className="text-white font-semibold">
                  {modules.find(m => m.id === currentModule)?.progress || 0}%
                </span>
              </div>
              <Progress value={modules.find(m => m.id === currentModule)?.progress || 0} className="h-3" />
              ...
              <div className="flex items-center justify-between">
                ...
                <div className="flex flex-col items-end space-y-2">
                  ...
                  <div className="flex items-center space-x-1">
                    ...
                    <span className="text-yellow-400 text-sm">{module.completionReward}</span>
                  </div>
                </div>
              </div>
            </CardHeader>
