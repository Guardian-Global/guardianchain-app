...
export default function MicroInteractionWalletConnection() {
  ...
  const getStatusIcon = (status: string) => {
    switch (status) {
      ...
      case 'connecting': return <Zap className="h-4 w-4 text-blue-400 animate-pulse" />;
      case 'verifying': return <Shield className="h-4 w-4 text-yellow-400 animate-spin" />;
  ...
  const resetConnection = () => {
    setInteractions(WALLET_MICRO_INTERACTIONS.map(interaction => ({
      ...interaction,
  ...
  const overallProgress = interactions.reduce((sum, interaction) => 
    sum + interaction.progress, 0) / interactions.length;

  const allConnected = interactions.every(interaction => 
        ...
        <CardContent className="space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div className="bg-slate-700/50 rounded-lg p-4 text-center">
              <p className="text-2xl font-bold text-blue-400">{Math.round(overallProgress)}%</p>
            ...
            <div className="bg-slate-700/50 rounded-lg p-4 text-center">
              <p className="text-2xl font-bold text-purple-400">{interactions.length}</p>
          ...
          {isConnecting && (
            <div className="space-y-2">
              ...
              <p className="text-center text-sm text-blue-400 animate-pulse">
                Step {connectionStep + 1} of {interactions.length}: {interactions[connectionStep]?.name}
              </p>
            </div>
          )}
              ...
              <div className="flex items-center justify-between">
                <div className="flex items-center space-x-4">
                  ...
                  <div className="flex-1">
                    <h3 className="text-white font-semibold">{interaction.name}</h3>
                    <p className="text-sm text-slate-400">{interaction.message}</p>
                  </div>
                </div>
                
                <div className="flex items-center space-x-3">
                  <Badge className={`text-xs ${
                    interaction.status === 'connected' ? 'bg-green-600' :
                    interaction.status === 'connecting' ? 'bg-blue-600' :
                    interaction.status === 'error' ? 'bg-red-600' : 'bg-gray-600'
                  }`}>
