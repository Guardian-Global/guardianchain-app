...
const walkthroughData: Record<string, { title: string; description: string; steps: WalkthroughStep[] }> = {
  '/create': {
    ...
    steps: [
      ...
      },
      {
        ...
        description: 'Your content receives a grief tier that determines its yield potential. Higher tiers earn more GTT.',
    ...
    steps: [
      {
        ...
        description: 'Select the type of eternal contract. Digital wills, testimonies, and declarations each serve different purposes.',
      ...
      },
      {
        ...
        description: 'Craft your eternal message. This content will be permanently stored on the blockchain.',
      ...
      },
      {
        ...
        title: 'Seal Forever',
        description: 'Once sealed, your contract becomes immutable and permanently stored on-chain.',
    ...
    steps: [
      {
        ...
        description: 'Track your total GTT earnings, active yield, and performance metrics across all capsules.',
      ...
      },
      {
        ...
        description: 'Understand which capsules generate the most value and engagement over time.',
      ...
      },
      {
        ...
        description: 'Stake your GTT tokens for enhanced returns and voting power in DAO governance.',
      ...
      },
      {
        ...
        description: 'Use our calculator to project earnings based on different capsule types and staking strategies.',
...
export default function NavigationProvider({ children }: NavigationProviderProps) {
  ...
  const [currentWalkthroughPath, setCurrentWalkthroughPath] = useState<string | null>(null);
  const [currentStep, setCurrentStep] = useState(0);
  const [completedSteps, setCompletedSteps] = useState<string[]>([]);
  const [tooltipState, setTooltipState] = useState<{ target: string; content: string } | null>(null);

  // Auto-start walkthrough for specific paths
  useEffect(() => {
    if (walkthroughData[location] && !completedSteps.includes(`walkthrough-${location}`)) {
  ...
  useEffect(() => {
    localStorage.setItem('guardianchain-completed-steps', JSON.stringify(completedSteps));
  }, [completedSteps]);
  ...
  const nextStep = () => {
    const walkthrough = currentWalkthroughPath ? walkthroughData[currentWalkthroughPath] : null;
  ...
  const currentWalkthrough = currentWalkthroughPath ? walkthroughData[currentWalkthroughPath] : null;
