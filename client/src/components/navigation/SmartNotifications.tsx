...
interface SmartNotification {
  id: string;
  type: 'achievement' | 'milestone' | 'social' | 'economic' | 'reminder' | 'opportunity';
  title: string;
  message: string;
  action?: {
    label: string;
  ...
  '/': [
    {
      ...
      message: 'Your truth vault awaits. Check your yield earnings and recent capsule performance.',
  ...
  '/create': [
    {
      ...
      message: 'Personal memories with emotional depth typically earn 40% higher grief scores.',
      priority: 'medium',
      timestamp: new Date().toISOString(),
      read: false,
...
export default function SmartNotifications() {
  ...
  const generateGlobalNotifications = (): SmartNotification[] => {
    ...
    if (hour >= 18 && hour <= 22) {
      notifications.push({
        ...
        message: 'Perfect time to capture today\'s thoughts in an eternal capsule.',
    ...
    if (Math.random() > 0.7) {
      notifications.push({
        ...
        message: '127 Guardians are actively creating capsules right now. Join the movement!',
  ...
  const getNotificationIcon = (type: SmartNotification['type']) => {
    switch (type) {
      case 'achievement': return <Star className="h-4 w-4 text-brand-accent" />;
      case 'milestone': return <Target className="h-4 w-4 text-brand-primary" />;
      case 'social': return <Users className="h-4 w-4 text-blue-400" />;
      case 'economic': return <Coins className="h-4 w-4 text-green-400" />;
      case 'reminder': return <Clock className="h-4 w-4 text-orange-400" />;
      case 'opportunity': return <Zap className="h-4 w-4 text-purple-400" />;
