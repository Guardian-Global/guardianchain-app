...
export default function PersonalizationEngine() {
  ...
  const generateRecommendations = (userPersonality: UserPersonality) => {
    ...
    if (userPersonality.traits.explorer > 60) {
      recs.push({
        ...
        description:
          "Explore the interactive map to find Guardians in your area and see their reputation levels.",
        action: { label: "Explore Map", path: "/guardian-map" },
        priority: 85,
        reasoning:
          "High explorer trait detected - you love discovering new features",
    ...
    if (userPersonality.traits.creator > 60) {
      recs.push({
        ...
        description:
          "Craft an eternal contract that will preserve your wishes and wisdom forever.",
    ...
    if (userPersonality.traits.analytical > 60) {
      recs.push({
        ...
        description:
          "Deep dive into your capsule performance metrics and optimize your earning strategy.",
        action: { label: "View Analytics", path: "/dashboard/yield" },
        priority: 80,
        reasoning:
          "High analytical trait - you appreciate data-driven insights",
    ...
    if (userPersonality.traits.social > 60) {
      recs.push({
        ...
        description:
          "Use your GTT voting power to shape the future of GuardianChain governance.",
        action: { label: "View Proposals", path: "/dao" },
        priority: 75,
        reasoning:
          "Strong social engagement - you like community participation",
        category: "social",
    ...
    if (userPersonality.journey.stage === "newcomer") {
      recs.push({
        ...
        description:
          "New users get 2x grief scoring bonus on their first capsule. Start your truth journey!",
        action: { label: "Create Capsule", path: "/create" },
        priority: 100,
        reasoning:
          "Newcomer bonus available - maximize your first capsule value",
        category: "next_step",
    ...
    if (userPersonality.preferences.riskTolerance === "aggressive") {
      recs.push({
        ...
        description:
          "Lock your GTT for 100 days and earn up to 25% APY with sovereignty tier benefits.",
        action: { label: "Stake Tokens", path: "/dashboard/yield" },
        priority: 70,
        reasoning:
          "Aggressive risk tolerance - you're open to high-reward opportunities",
        category: "earning",
  ...
  const getCategoryIcon = (
    ...
    switch (category) {
      ...
      case "optimization":
        return <TrendingUp className="h-4 w-4 text-brand-accent" />;
      case "exploration":
            ...
            <div>
              ...
              <div className="flex items-center gap-2 mb-2">
                ...
                <span className="text-slate-400 text-xs">
                  {personality.journey.completedActions.length} actions
                  completed
                </span>
              </div>
              <Progress
                value={(personality.journey.completedActions.length / 10) * 100}
                className="h-1"
              />
            </div>
            ...
            <div>
              ...
              <div className="space-y-2">
                {recommendations.map((rec) => (
                  <div
                    key={rec.id}
                    className="bg-slate-700/30 p-3 rounded border-l-2 border-brand-primary/50"
                  >
                    <div className="flex items-start gap-2 mb-2">
                      {getCategoryIcon(rec.category)}
                      <div className="flex-1">
                        <h5 className="text-white text-xs font-medium leading-tight">
                          {rec.title}
                        </h5>
