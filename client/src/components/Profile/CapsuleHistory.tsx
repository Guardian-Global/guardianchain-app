...
export default function CapsuleHistory({ userAddress }: CapsuleHistoryProps) {
  const {
    ...
    queryFn: () =>
      userAddress ? fetchUserCapsules(userAddress) : Promise.resolve([]),
    enabled: !!userAddress,
  });
        ...
        <CardTitle className="flex items-center gap-2">
          ...
          <div>
            ...
            <div className="text-sm text-slate-400">
              {capsules.length} total capsules created
            </div>
          </div>
        </CardTitle>
      </CardHeader>
        ...
        <div className="space-y-4">
          ...
          <div className="space-y-3">
            {capsules.map((capsule) => (
              <Card
                key={capsule.id}
                className="bg-slate-700/30 border-slate-600 hover:bg-slate-700/40 transition-colors"
              >
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
            ...
            <div>
              ...
              <div className="text-white font-semibold">
                {(
                  capsules.reduce((sum, cap) => sum + cap.griefScore, 0) /
                  capsules.length
                ).toFixed(1)}
                %
              </div>
