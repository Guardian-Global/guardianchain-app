...
import { 
  User, Trophy, Target, Coins, Shield, Award, 
  GitBranch, Eye, Share2, Vote, TrendingUp, 
  Calendar, ExternalLink, Edit3, Settings 
} from 'lucide-react';
...
export default function EnhancedProfile({ user, isOwnProfile, onEdit }: EnhancedProfileProps) {
      ...
      <div className="container mx-auto px-4 py-8 max-w-6xl">
          ...
          <CardContent className="p-8">
            <div className="flex flex-col md:flex-row items-start gap-6">
              ...
              <div className="flex flex-col items-center md:items-start">
                ...
                <div className="text-center md:text-left mt-4">
                  <h1 className="text-2xl font-bold text-white flex items-center gap-2">
                    {user.username}
                    {user.isVerified && <Badge className="bg-green-600 text-white">Verified</Badge>}
                  </h1>
              ...
              <div className="flex-1 grid grid-cols-2 md:grid-cols-4 gap-4">
                ...
                <div className="text-center">
                  <div className="text-2xl font-bold text-yellow-400">{user.gttBalance}</div>
            ...
            <div className="mt-6 bg-slate-700/30 rounded-lg p-4">
              <div className="flex items-center justify-between mb-2">
                ...
                <span className="text-slate-400 text-sm">{user.xpPoints}/{nextLevelXP} XP</span>
              </div>
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-6">
          <TabsList className="bg-slate-800 border-slate-700">
            ...
            <TabsTrigger value="capsules" className="data-[state=active]:bg-purple-600">
              Capsules
            </TabsTrigger>
                ...
                <CardContent className="space-y-4">
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                    <div className="text-center p-3 bg-slate-700/30 rounded-lg">
                      ...
                      <div className="text-lg font-bold text-white">{user.stats.totalViews.toLocaleString()}</div>
                      <div className="text-slate-400 text-xs">Total Views</div>
                    </div>
                    <div className="text-center p-3 bg-slate-700/30 rounded-lg">
                      <Share2 className="w-5 h-5 text-green-400 mx-auto mb-1" />
                      <div className="text-lg font-bold text-white">{user.stats.totalShares}</div>
                    ...
                    <div className="text-center p-3 bg-slate-700/30 rounded-lg">
                      ...
                      <div className="text-lg font-bold text-white">{user.stats.accuracyRate}%</div>
                  ...
                  <div className="space-y-3">
                    <div>
                      <div className="flex justify-between text-sm mb-1">
                        <span className="text-slate-400">DAO Participation</span>
                        <span className="text-white">{user.stats.daoParticipation}%</span>
                      </div>
                      <Progress value={user.stats.daoParticipation} className="h-2" />
                    </div>
                  </div>
                </CardContent>
              </Card>
            ...
            <Card className="bg-slate-800/50 border-slate-700">
              ...
              <CardContent>
                <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-7 gap-3">
                  {Object.entries(user.stats.capsulesByType).map(([type, count]) => (
                    <div key={type} className="text-center p-3 bg-slate-700/30 rounded-lg">
                      <div className="text-lg font-bold text-white">{count}</div>
                      <div className="text-slate-400 text-xs capitalize">{type.toLowerCase()}</div>
                    </div>
                  ))}
                </div>
              </CardContent>
              ...
              <CardContent>
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                  {user.achievements.map((achievement) => (
                    <Card key={achievement.id} className="bg-slate-700/30 border-slate-600">
