...
export default function DynamicMeta({ 
  ...
  useEffect(() => {
    ...
    const metaDescription = capsule
      ...
      : description || "Decentralized truth verification platform with blockchain-sealed capsules and community governance.";
      
    const metaImage = capsule?.imageUrl 
      || image 
      || `https://api.dicebear.com/7.x/shapes/svg?seed=${capsule?.id || 'guardianchain'}&backgroundColor=1e293b`;
    ...
    updateMetaTag('keywords', 'blockchain, truth verification, NFT, Web3, decentralized, immutable proof, DocuSign', true);
    ...
    if (capsule) {
      ...
      updateMetaTag('article:published_time', capsule.createdAt || new Date().toISOString());
      updateMetaTag('article:modified_time', capsule.updatedAt || capsule.createdAt || new Date().toISOString());
