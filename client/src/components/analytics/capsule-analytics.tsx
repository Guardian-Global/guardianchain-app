...
export default function CapsuleAnalytics({ 
  ...
  const { data: analytics, isLoading } = useQuery<AnalyticsData>({
    ...
    queryFn: async () => {
      const response = await apiRequest("GET", `/api/analytics/${capsule.id}/analytics`);
  ...
  const claimYieldMutation = useMutation({
    mutationFn: async () => {
      ...
      const response = await apiRequest("POST", `/api/analytics/${capsule.id}/claim-yield`, {
        walletAddress
      });
      return response.json();
    },
    onSuccess: (data) => {
      ...
      queryClient.invalidateQueries({ queryKey: ['/api/analytics', capsule.id] });
  ...
  const canClaim = metrics.truthYield >= 1.0 && showClaimButton && walletAddress;
        ...
        <CardContent>
          <div className="space-y-4">
            ...
            <div className="flex items-center gap-3">
              ...
              <span className="text-sm text-slate-400">{yieldTier.description}</span>
            </div>

            {/* Main Metrics */}
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              <div className="bg-slate-900 p-3 rounded-lg text-center">
                <div className="text-2xl font-bold text-white">{metrics.truthYield}</div>
                <div className="text-xs text-slate-400">Truth Yield</div>
              </div>
              <div className="bg-slate-900 p-3 rounded-lg text-center">
                <div className="text-2xl font-bold text-green-400">{metrics.gttReward}</div>
                <div className="text-xs text-slate-400">GTT Reward</div>
              </div>
              <div className="bg-slate-900 p-3 rounded-lg text-center">
                <div className="text-2xl font-bold text-blue-400">{metrics.griefScore}</div>
                <div className="text-xs text-slate-400">Grief Score</div>
              </div>
              <div className="bg-slate-900 p-3 rounded-lg text-center">
                <div className="text-2xl font-bold text-purple-400">{growth.daysActive}</div>
            ...
            {canClaim && isCreator && (
              ...
              >
                ...
                {isClaimingYield ? "Claiming..." : `Claim ${metrics.gttReward} GTT`}
              </Button>
            )}
        ...
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                <div className="text-lg font-semibold text-white">{metrics.views}</div>
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <div className="text-lg font-semibold text-white">{metrics.shares}</div>
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <div className="text-lg font-semibold text-white">{metrics.verifications}</div>
                <div className="text-sm text-slate-400">Verifications</div>
                <div className="text-xs text-slate-500">Community validated</div>
              </div>
            </div>
          </div>
        </CardContent>
        ...
        <CardContent>
          <div className="space-y-3">
            ...
            <div className="flex justify-between items-center">
              ...
              <Badge className={
                metrics.status === 'sealed' ? 'bg-purple-600' :
                metrics.status === 'verified' ? 'bg-green-600' :
                'bg-blue-600'
              }>
                {metrics.status === 'sealed' ? 'Veritas Sealed' : 
                 metrics.status === 'verified' ? 'Truth Verified' : 
                 'Pending Review'}
              </Badge>
            </div>
            ...
            <div className="flex justify-between items-center">
              ...
              <span className={`font-semibold ${metrics.minted ? 'text-green-400' : 'text-slate-400'}`}>
        ...
        <CardContent>
          <div className="space-y-2 text-sm">
            <div className="flex justify-between">
              <span className="text-slate-400">Views ({metrics.views} × 0.5)</span>
              <span className="text-white">{(metrics.views * 0.5).toFixed(1)}</span>
            </div>
            <div className="flex justify-between">
              <span className="text-slate-400">Shares ({metrics.shares} × 1.5)</span>
              <span className="text-white">{(metrics.shares * 1.5).toFixed(1)}</span>
            </div>
            <div className="flex justify-between">
              <span className="text-slate-400">Verifications ({metrics.verifications} × 3.0)</span>
              <span className="text-white">{(metrics.verifications * 3.0).toFixed(1)}</span>
            </div>
            {metrics.minted && (
              <div className="flex justify-between">
