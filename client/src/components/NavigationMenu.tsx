        ...
        <Card className="w-full max-w-4xl bg-black/90 backdrop-blur-xl border-purple-500/20">
          <CardContent className="p-6">
            ...
            <div className="flex items-center justify-between mb-6">
              <div>
                <h2 className="text-2xl font-bold text-purple-300">GuardianChain Navigation</h2>
                <p className="text-gray-400">Explore the truth preservation ecosystem</p>
              </div>
              <Button
            ...
            {isAuthenticated && (
              <div className="bg-slate-800/50 rounded-lg p-3 mb-6">
                <div className="flex items-center justify-between">
                  <div>
                    <p className="text-white font-medium">Welcome back, {(user as any)?.firstName || 'Guardian'}</p>
              ...
              {Object.entries(groupedItems).map(([category, items]) => (
                <div key={category}>
                  <h3 className="text-lg font-semibold text-gray-300 mb-3 flex items-center">
                    ...
                    {category === 'Advanced' && <Star className="w-5 h-5 mr-2" />}
                    {category === 'Social' && <Users className="w-5 h-5 mr-2" />}
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
                    {items.map((item) => {
                      ...
                      return (
                        <Button
                          key={item.path}
                          onClick={() => accessible ? handleNavigation(item.path) : null}
                          variant="ghost"
