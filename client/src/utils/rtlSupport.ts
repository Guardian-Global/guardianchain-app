...
export const RTL_LANGUAGES = [
  'ar', 'he', 'fa', 'ur', 'ps', 'sd', 'ku', 'yi', 'ji', 'iw', 
  'arc', 'bcc', 'bqi', 'ckb', 'dv', 'fa-AF', 'fa-IR', 'glk', 
  'he-IL', 'khw', 'ks', 'lrc', 'mzn', 'pnb', 'ps-AF', 'ps-PK', 
  'sd-IN', 'sd-PK', 'ug', 'ur-IN', 'ur-PK'
];
...
export function getTextAlignment(language: string): 'left' | 'right' | 'center' {
  return isRTL(language) ? 'right' : 'left';
}

export function getMarginDirection(language: string): { marginLeft?: string, marginRight?: string } {
  return isRTL(language) 
    ? { marginRight: 'auto' } 
    : { marginLeft: 'auto' };
}

export function getPaddingDirection(language: string, size: string = '1rem'): { paddingLeft?: string, paddingRight?: string } {
  return isRTL(language)
    ? { paddingRight: size }
    : { paddingLeft: size };
}