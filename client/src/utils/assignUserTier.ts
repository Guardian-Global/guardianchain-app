...
export async function assignUserTier(
  userId: string,
  newTier: "guest" | "explorer" | "pro" | "admin" | "enterprise",
): Promise<boolean> {
  try {
    ...
    console.log(
      `✅ Assigned '${newTier}' tier to user ${userId} (was: ${previousTier || "none"})`,
    );
    ...
    const existingLogs = JSON.parse(
      localStorage.getItem("tier-audit-log") || "[]",
    );
...
export async function assignUserTierProduction(
  userId: string,
  newTier: string,
...
export async function bulkAssignTiers(
  assignments: Array<{ userId: string; tier: string }>,
): Promise<{ success: number; failed: number }> {
  let success = 0;
  let failed = 0;

  for (const assignment of assignments) {
    const result = await assignUserTier(
      assignment.userId,
      assignment.tier as any,
    );
    if (result) {
      success++;
    } else {
...
export function getTierAuditLog(): Array<{
  userId: string;
  previousTier: string;
  newTier: string;
  assignedAt: string;
