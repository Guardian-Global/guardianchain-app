...
function generateProofData(capsule: CapsuleProofData) {
  return {
    ...
    legal: {
      ...
      disclaimer: 'This certificate represents community consensus and does not constitute legal proof.',
...
function calculateTrustScore(capsule: CapsuleProofData): number {
  ...
  switch (capsule.verificationLevel) {
    case 'sovereign': score += 40; break;
    case 'professional': score += 30; break;
    case 'community': score += 20; break;
    default: score += 0;
  }
  ...
  const daysSinceSealing = (Date.now() - new Date(capsule.sealedAt).getTime()) / (1000 * 60 * 60 * 24);
...
function generatePDFContent(doc: any, proofData: any): void {
  ...
  doc.text(`Generated: ${new Date(proofData.generatedAt).toLocaleString()}`, 20, 270);
